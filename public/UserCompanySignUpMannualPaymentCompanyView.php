...
<script>
 var oldMonth='<?php echo $oldMonth; ?>';
 ...
 function updateExpiryMonth(id)
 {
	 if(id==yearNow)
	 {
		 $('#expiry_month').html(oldMonth);
	 }
	 else
		 {
		 $('#expiry_month').html(nextMonth);
	 }
 }
	    function submitform()
		{
			...
			if($("#company_id").val()==1)
			{
			 $("#errorMsg1").html('Company already exist please use worq app to book using company ');
			 return false;
			 }
			if($("#user_id").val()==0)
			{
			...
			if($("#pnumber").val()==0 || $("#pnumber").val()=="")
			{
			 ...
			 if($("#card_number").val()=="")
			  ...
			  if($("#cvv").val().length!=3)
				...
				$("#errorMsg1").html("please enter 3 digit numeric value for cvv number !!!");
		 ...
		 if(isNaN($("#cvv").val()))
				...
				$("#errorMsg1").html("please enter numeric value for cvv number !!!");
			 ...
			 if($("#user_id").val()==0)
			 {
				 if($("#ev_code").val()==0 || $("#ev_code").val()=="")
				{
					
					$("#errorMsg1").html('Please enter email verification code');
					return false;
				}
				 if($("#v_code").val()==0 || $("#v_code").val()=="")
				{
					
					$("#errorMsg1").html('Please enter phone verification code');
					return false;
				}
			
				 var send_data={};
				send_data.country_id=$("#country").val();
					send_data.phone_number=$("#pnumber").val();
					send_data.v_code=$("#v_code").val();
					send_data.ev_code=$("#ev_code").val();
					$.ajax({
						type:"POST",
						url:"../../verifyEmailPhoneCodeInfo/<?php echo $data['job_id']; ?>",
						data:send_data,
						dataType:"text",
						...
						if(data1==0)
						{
						$("#errorMsg1").html('Please enter correct code');
						return false;	
						}
						else
						{
			 ...
			 if($("#ev_code").val()==0 || $("#ev_code").val()=="")
			{
				...
				send_data.country_id=$("#country").val();
					...
					$.ajax({
						type:"POST",
						url:"../../verifyEmailCodeInfo/<?php echo $data['job_id']; ?>",
						data:send_data,
						dataType:"text",
						contentType: "application/x-www-form-urlencoded;charset=utf-8",
						success: function(data1){
		...
		function checkEmailInfo(id)
		{
			...
			send_data.email=$("#email").val();
					...
					$.ajax({
						type:"POST",
						url:"../../verifyEmail/<?php echo $data['job_id']; ?>",
						data:send_data,
						dataType:"text",
						contentType: "application/x-www-form-urlencoded;charset=utf-8",
						success: function(data1){
							
						$('.checkout__button').removeClass('hidden'); 
						$('#CompanyDetails').removeClass('hidden'); 
						$('#cardDetail').removeClass('hidden'); 
						$('#emailcodeValue').removeClass('hidden'); 
						$('#changeMargin').addClass('marb927');
						$('#changeMargin').removeClass('marb175');
						$('#user_id').val(data1);
						if(data1==0)
						{
							$("#user_id").val(data1);
	...
	function checkCompanyIdInfo(id)
		{
			$("#errorMsg1").html('');
			if($("#cid").val()=="" || $("#cid").val()=="")
			{
			...
			send_data.cid=$("#cid").val();
					...
					$.ajax({
						type:"POST",
						url:"../../verifyCompanyIdentification/<?php echo $data['job_id']; ?>",
						data:send_data,
						dataType:"text",
						contentType: "application/x-www-form-urlencoded;charset=utf-8",
						success: function(data1){
		...
		function checkPhoneNumber(id)
		{
			var send_data={};
			send_data.country_id=$("#country").val();
					send_data.phone_number=id;
					$.ajax({
						type:"POST",
						url:"../../sendVerificationCode/<?php echo $data['job_id']; ?>",
						data:send_data,
						dataType:"text",
						contentType: "application/x-www-form-urlencoded;charset=utf-8",
						success: function(data1){
						$('#verified_number').val(0);
						$('#codeValue').removeClass('hidden');	  
						}
					}); 
		}
...
<a href="../../signUpJobEmailQrInfo/<?php echo $data['domain_id']; ?>/<?php echo $data['job_id']; ?>" class="header__item  " style="margin-left:10px;"><div class="header__item_language js-header-item" style="border-left: 0px solid !important; ">
<button class="header__head js-header-head fsz16 hidden-xs changedText">Scan QR Code</button>
...
<div class="section main main_stays" style="margin-bottom:80px;">
         ...
         <div class="travel__list" style="
            ...
            <div class="travel__item">
               ...
               <div class="travel__subtitle changedText" style="
                  ...
                  ">Welcome to <?php echo $selectedMarketplaceDetail['marketplace_name']; ?>
</div>
               
            <div class="travel__content changedText " style="
                  color: #787f91;
                  font-size: 20px;
                  ">Get started on your journey of finding the perfect broker for your project.
</div><div class="product__rating  xs-padb10 xs-brdrbf67f00" style="
    padding-top: 25px;
">
                    ...
                    <div class="product__reviews ">(256 <span class="changedText">reviews</span>)</div>
                  </div>

<div class="travel__col xs-marb0 marb175" style="padding-right: 0px; max-width:525px;" id="changeMargin">
         <div class="travel__list" style="
            max-width: 450px;
...
<div class="categories__center center1 xsi-nobrdb xsi-padb0 padb20"  >
<div class="categories__inner">

 <div class="categories__wrapper">
               <div class="categories__slider js-slider-categories" style="padding-bottom:0px;">
 <a class="categories__item black_bg " href="javascript:void(0);" style="padding: 20px; font-size:14px; border-radius:6px; ">
                                       
<div class="changedText categories__subtitle  txt_f67f00 ">Company</div>
  </a>
  
   <a class="categories__item  trans_bg " href="../../signUpJobEmailInfo/<?php echo $data['domain_id']; ?>/<?php echo $data['job_id']; ?>" style="padding: 20px; font-size:14px; border-radius:6px; background:transparent;  border:none;">
...
<script>
function updateUserFields(id)
{
			if(id==1)
				{
					window.location.href='https://www.qloudid.com/public/index.php/UserCompanySignUp/signUpJobEmailInfo/<?php echo $data['domain_id']; ?>/<?php echo $data['job_id']; ?>';
				}
	 
}
</script>
<div class="checkout__section xsi-nobrdt padt32" style="margin-top:10px;   padding-bottom:136px; ">
<form class="checkout__item js-tabs-item" action="../../updateCompanyPaymentDetail/<?php echo $data['domain_id']; ?>/<?php echo $data['job_id']; ?>" method="POST" id="save_indexing" name="save_indexing" accept-charset="ISO-8859-1"  style="display: block;">
 <input type="hidden"  value="2"  name="carried_for" id="carried_for" />
<div class="checkout__fieldset subscription subscription_big">
  
<div class="marb40">
 
<input class="subscription__input" type="text" name="email" id="email" placeholder="Simply enter personal email" required="required" personal="">
  
<div class="subscription__btn " style="background: #f67f00;" onclick="checkEmailInfo(this.value);">
<svg class="icon icon-search mar10" style="
    fill: black;
">
...
<div class="field hidden" id="emailcodeValue" style="margin-bottom:24px;">
<div class="field__label changedText" style="color:#787f91;">Email Verification code</div>
<div class="field__wrap">
<input class="field__input"  name="ev_code" id="ev_code"    type="text"   style=" color:grey;"  >
</div>
</div>

<div class="hidden" id="userDetail">

<div class="checkout__row" style="margin-bottom:24px;">
<div class="field">
<div class="field__label changedText" style="color:#787f91;">First name</div>
<div class="field__wrap">
<input class="field__input" name="fname" id="fname"    type="text"   placeholder="e. g. Kowaken"  style=" color:grey;">
</div>
</div>
<div class="field">
<div class="field__label changedText" style="color:#787f91;">Last name</div>
<div class="field__wrap">
<input class="field__input"  name="lname" id="lname"    type="text"  placeholder="e. g. Ghirmai" style=" color:grey;">
</div>
</div>
</div>


<div class="upload__row " style="margin-bottom:0px;">
  <div class="upload__col upload__col_w30">
<div class="field">
<div class="field__label changedText" style="color:#787f91;">Country</div>
<div class="field__wrap">
 
<select  class="   "  name="country" id="country" style="height: 48px;
...
<div class="field__label changedText" style="color:#787f91;">MOBILE - (DONT USE FIRST 0)</div>
<div class="field__wrap">
<input class="field__input"  name="pnumber" id="pnumber"    type="text"  placeholder="e. g. xyz@gmail.com"  style=" color:grey;" onfocusout="checkPhoneNumber(this.value);">
</div>
</div>
</div>
</div>
...
<div class="field hidden" id="codeValue" style="margin-bottom:24px;">
<div class="field__label changedText" style="color:#787f91;">Verification code</div>
<div class="field__wrap">
<input class="field__input"  name="v_code" id="v_code"    type="text"   style=" color:grey;"  >
</div>
</div>
 
 <div class="checkout__fieldset hidden" id="CompanyDetails" style="border-top: 2px solid #353945; padding-top:24px; margin-bottom:40px;">
<div class="field" style="margin-bottom:24px;" >
<div class="field__label" style="color:#787f91;">Company name</div>
<div class="field__wrap">
<input class="field__input"  type="text" name="company_name" id="company_name" placeholder="e. g. Qloud ID Inc."  required="" style=" color:grey;">
</div>
</div>

<div class="checkout__fieldset"  >
<div class="field"  style="margin-bottom:24px;">
<div class="field__label" style="color:#787f91;">Company identification</div>
<div class="field__wrap">
<input class="field__input"  type="text" name="cid" id="cid"    required="" style=" color:grey;" placeholder="e. g. 1234567"  onfocusout="checkCompanyIdInfo(this.value);">
</div>
</div>
<input type="hidden" id="company_id" name="company_id" value="0" />
<input type="hidden" id="company_type" name="company_type" value="1" />
<input type="hidden" id="company_email" name="company_email" value="" />
<input type="hidden" id="objectType" name="objectType" value="1">
<input type="hidden" id="signup_type" name="signup_type"  value="2" />
<div class="upload__row " style="margin-bottom:24px;">
  <div class="upload__col upload__col_w70">
<div class="field" >
<div class="field__label" style="color:#787f91;">Street address</div>
<div class="field__wrap">
<input class="field__input" name="daddress" id="daddress"    type="text"  placeholder="e. g. Mjorbekan"  style=" color:grey;">
</div>
</div>
</div>
<div class="upload__col upload__col_w30">
...
<div class="field__wrap">
<input class="field__input"  name="d_port_number" id="d_port_number"  placeholder="e. g. 121"   type="text"   style=" color:grey;">
</div>
</div>
</div>
 </div>
...
<div class="field__wrap">
<input class="field__input" name="dzip" id="dzip"    type="text" placeholder="e. g. 111100"   style=" color:grey;">
</div>
</div>
</div>
<div class="upload__col upload__col_w70">
...
<div class="field__wrap">
<input class="field__input"  name="dcity" id="dcity"    type="text" placeholder="e. g. Stockholam"   style=" color:grey;">
</div>
</div>
</div>
</div>
...
<div class="checkout__line  " style="margin-bottom:24px;">
					   <div class="checkout__subtitle changedText">Card</div>
                       
                      </div>
 
<div class="field" style="margin-bottom:24px;">
<div class="field__label changedText" style="color:#787f91;">Card number</div>
<div class="field__wrap">
<input class="field__input" name="card_number" id="card_number"  placeholder="e. g. xxxxxxxxxxxxxxxx"   type="text"    >
</div>
</div>

 <div class="checkout__row" style="margin-bottom:24px;">
<div class="field "  >
<div class="field__label changedText" style="color:#787f91;">Expiry year</div>
 <div class="field__wrap">
<select class=" " id="expiry_year" name="expiry_year" style="height: 48px;
    padding: 0 14px; border-color: #353945;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;" fdprocessedid="8mc52" onchange="updateExpiryMonth(this.value);">
				 	<?php $i=date('Y'); for($j=0; $j<=11; $j++) { ?>
                           <option value="<?php echo $j+$i; ?>" class="lgtgrey2_bg changedText"><?php echo $j+$i; ?></option>
...
<div class="field"  >
<div class="field__label changedText" style="color:#787f91;">Expiry month</div>
<div class="field__wrap">
<select class=" " id="expiry_month" name="expiry_month" style="height: 48px;
    padding: 0 14px; border-color: #353945;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;" fdprocessedid="8mc52">
				 	<?php $i=(int)date('m'); for($j=$i; $j<=12; $j++) { ?>
                           <option value="<?php echo $j; ?>" class="lgtgrey2_bg changedText"><?php echo $j; ?></option>
 ...
 <div class="field">
<div class="field__label changedText" style="color:#787f91;">Cvv</div>
<div class="field__wrap">
<input class="field__input"  name="cvv" id="cvv"    type="text" placeholder="e. g. 121"   >
</div>
</div>
 
<div class="checkout__fieldset mart35">
 <input type="hidden" id="user_id" name="user_id"  value="0" /> 
 
 
<a class="button checkout__button changedText hidden" href="javascript:void(0);"  onclick="submitform();" >Submit</a>
 ...
 <div class="travel__center center   padb136  xs-padrl25 mart80 xs-mart25i" style="
    max-width: 1132px;
    padding-right: 0px;
    padding-left: 0px;
    ">
