	   ...
	   function updateInclutionType(id)
	   {
	    ...
	    function submitform()
		{
			 ...
			 if($("#email").val()=="" || $("#email").val()=="")
			{
			 ...
			 if($("#pcountry").val()==0 || $("#pcountry").val()=="")
			{
		 ...
		 if($("#company_name").val()=="" || $("#company_name").val()=="")
			...
			if($("#objectType").val()==0)
			{
				...
				$("#errorMsg1").html('Please select one service you will provide');
				return false;
			}
			 else 
			{
		...
		function checkCompnayId()
		{
			...
			var send_data={};
			if($('#personal_account').val()==1)
			{
			send_data.email=$("#email").val();	
			}
			else
			{
				send_data.email=$("#pemail").val();
			}
			 
			send_data.cid=$("#cid").val();
			$.ajax({
						type:"POST",
						url:"../verifyCompanyIdDetail/<?php echo $data['a_id']; ?>",
						data:send_data,
						dataType:"text",
						...
						success: function(data1){
							...
							if(data1==2)
							{
								$("#company_exist_info").val(data1);
								$("#errorMsg1").html('You are not an employee of this company');
								return false;
							}
							else if(data1==0)
							{
								$("#company_exist_info").val(data1);
								$('#companyDetail').removeClass('hidden');
...
<body class="dark" style="background: rgb(19, 25, 34);">
   ...
   <div class="travel__row " style="margin-bottom: 15px;">
      <div class="travel__col " style="padding-right: 0px;  max-width:595px; margin-bottom:0px;">
         <div class="travel__list" style="
            max-width: 425px;
            ">
            <div class="travel__item xs-marb40 marb422">
            ...
            <div class="travel__content changedText xs-fsz27" style=" line-height: 1.4;
                  color: #787f91;
				   line-height: 1.4;
                  font-size: 27px;
                  ">Complete the fields below to validate your broker invitation.
</div>
...
<div class="checkout__category changedText" style="font-weight:normal !important;">Personal detais</div>
 



</div>
<form class="checkout__item js-tabs-item" action="../verifyOtpDetails/<?php echo $data['a_id']; ?>" method="POST" id="save_indexing" name="save_indexing" accept-charset="ISO-8859-1"  style="display: block;">
...
<input class="field__input xs-fieldData" type="text" name="email" id="email" placeholder="e. g. kowaken.ghirmai@gmail.com" value="<?php echo $invationInformation['email'];?>" readonly>
</div>
</div>
<input type="hidden" id="personal_account" name="personal_account" value="1"/>
<div class="setting__item  " style="display:block; padding-bottom:40px;">
                 
                <div class="setting__list">
                  <div class="setting__box">
<div class="setting__notification">
                      <div class="setting__main">
                        <div class="setting__subtitle"><?php echo $invationInformation['email'];?></div>
                        <label class="switch">
                          <input class="switch__input" type="checkbox" onclick='updatePersonal(this);'  checked /><span class="switch__inner"><span class="switch__box"></span></span>
                        </label>
                      </div>
                      <div class="setting__content">Is this your personal email address?</div>
                    </div>
					</div>
					</div>
					</div>
					<script>
									function updatePersonal(link)
										   {
											  body = $('body');
											   if($('#personal_account').val()==0)
											   {
												    $('#bodyData').addClass('dark');
												    $('#newEmail').addClass('hidden');
												     $(link).attr('checked',''); 
													$('#personal_account').val(1);	
...
<div class="field hidden" id="newEmail">
<div class="field__label changedText" style="color:#787f91;">What is your personal Email</div>
<div class="field__wrap">
<input class="field__input xs-fieldData" type="text" name="pemail" id="pemail" placeholder="e. g. xyz@gmail.com" value="" style="color:grey;  background:#282D34; border: 2px solid #f67f00;">
</div>
</div>


<div class="checkout__row" style="margin-bottom:32px;">
<div class="field">
<div class="field__label changedText" style="color:#787f91;">First name</div>
<div class="field__wrap">
<input class="field__input xs-fieldData" name="first_name" id="first_name"  value="<?php echo $invationInformation['first_name'];?>"  type="text" placeholder="e. g. XYZ"  style="color:grey; background:#282D34; border:none;" >
</div>
</div>
<div class="field">
<div class="field__label changedText" style="color:#787f91;">Last name</div>
<div class="field__wrap">
<input class="field__input xs-fieldData"  name="last_name" id="last_name"    type="text" placeholder="e. g. XYZ"  style="color:grey;  background:#282D34;  border:none;" value="<?php echo $invationInformation['last_name'];?>">
</div>
</div>
</div>

  
 <input class="field__input xs-fieldData" type="hidden" name="user_id" id="user_id" placeholder="e. g. 7222233554" value="<?php echo $invationInformation['user_id'];?>" >
<div class="upload__row" style="margin-bottom:32px;">
...
<select  class="xs-fieldData"  name="pcountry" id="pcountry" style="height: 48px;
    ...
    color: grey; border-radius: 12px;
      background:#282D34; width:100%;  border:none;" fdprocessedid="8mc52">
				  <?php  foreach($countryOptions as $category => $value) 
															{
																$category = htmlspecialchars($category); 
															?>
															
															<option value="<?php echo $value['id']; ?>" <?php if($value['id']==$invationInformation['country']) echo 'selected'; ?> class="lgtgrey2_bg changedText"><?php echo $value['country_name']; ?></option>
														<?php 	}	?>
...
<div class="field__wrap">
<input class="field__input xs-fieldData"  name="pnumber" id="pnumber"    type="text"  placeholder="e. g. xxxxxx1234" style="color:grey;  background:#282D34;  border:none;" value="<?php echo $invationInformation['phone_number'];?>">
</div>
</div>
</div>
 </div>
  ...
  <div class="upload__item">
					   ...
					   <div class="checkout__subtitle uppercase changedText">Business details</div>
                       
                      </div>
						  
						  <div class="field  margin-bottom24">
                            <div class="field__label  changedText" style="color:#787f91;">Company name</div>
                            <div class="field__wrap">
                              <input class="field__input xs-fieldData" type="text" name="company_name" id="company_name" placeholder="e. g. Qloud ID" style="color:grey;  background:#282D34;  border:none;" value="<?php echo $invationInformation['company_name'];?>">
                            </div>
                          </div>
						  
						     <div class="field  margin-bottom24">
                            <div class="field__label  changedText" style="color:#787f91;">Company identification</div>
                            <div class="field__wrap">
                              <input class="field__input xs-fieldData" type="text" name="cid" id="cid" placeholder="e. g. 123456789" style="color:grey;  background:#282D34;  border:none;" onfocusout="checkCompnayId();">
                            </div>
                          </div>
						  <div class="hidden" id="companyDetail">
...
<div class="upload__row">
                          <div class="upload__col upload__col_w70">
                        <div class="field  ">
                            <div class="field__label  changedText" style="color:#787f91;">Visiting address</div>
                            <div class="field__wrap">
                              <input class="field__input xs-fieldData" type="text" name="daddress" id="daddress" placeholder="e. g. Queenstown, Otago, New Zealand" style=" color:grey;  background:#282D34;  border:none;" fdprocessedid="hqixjq">
                            </div>
                          </div>
						  </div>
						   <div class="upload__col upload__col_w30">
						   <div class="field">
                            ...
                            <div class="field__wrap">
                              <input class="field__input xs-fieldData" type="text" name="d_port_number" id="d_port_number" placeholder="e. g. 121" style="color:grey;  background:#282D34;  border:none;" fdprocessedid="zw5lf">
                            </div>
                          </div>
						  </div>
						  </div>
...
<div class="field__wrap">
<input class="field__input xs-fieldData" name="dzip" id="dzip" type="text" placeholder="e. g. 111111" style="color:grey;  background:#282D34;  border:none;">
</div>
</div>
</div>
 <div class="upload__col upload__col_w70">
...
<div class="field__wrap">
<input class="field__input xs-fieldData" name="dcity" id="dcity" type="text" placeholder="e. g. Stockholam" style="color:grey;  background:#282D34;  border:none;">
</div>
</div>
</div>
</div>
 <input type="hidden" value="219"   id="objectType" name="objectType" >
 
			</div>			   
						 </div>
                       
               <input class="field__input xs-fieldData" type="hidden" name="company_exist_info" id="company_exist_info" value="0">       
  
  <div class="checkout__line  margin-bottom24 hidden">
					   <div class="checkout__subtitle uppercase changedText">REFERENCE detail</div>
                       
                      </div>
 <div class="field hidden">
<div class="field__label changedText" style="color:#787f91;">REFERENCE name</div>
<div class="field__wrap">
<input class="field__input xs-fieldData" type="text" name="refrence_name" id="refrence_name" placeholder="e. g. Ghirmai" required="" style=" color:grey;">
</div>
</div>
...
<div class="field__wrap">
<input class="field__input xs-fieldData" type="password" name="password" placeholder="Password" required="">
