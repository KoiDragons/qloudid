<?php
	...
	class NotifyFamilyFriendsModel extends AppModel
	{
		...
		function verifyOtp($data)
		{
			...
			$stmt = $dbCon->prepare("update notify_family_friends set country_name=?,phone_number=? where id=?");
		...
		function addInfo()
		{
			...
			$stmt = $dbCon->prepare("INSERT INTO notify_family_friends (person_heart_user_id, first_name, last_name, created_on ) VALUES (?, ?,  ?, now())");
				
				$st=0;
				$stmt->bind_param("iss", $_POST['user_id'],htmlentities($_POST['f_name'],ENT_NOQUOTES, 'ISO-8859-1'),htmlentities($_POST['l_name'],ENT_NOQUOTES, 'ISO-8859-1')); // ss -> string string
				if (!$stmt->execute()) {
					$stmt->close();
					$dbCon->close();
					return 0;
		...
		function uploadPhoto($data)
		{
			...
			$stmt = $dbCon->prepare("update notify_family_friends set image_path=? where id=?");
		...
		function updateAddress($data)
		{
			...
			$stmt = $dbCon->prepare("update notify_family_friends set address=? where id=?");
