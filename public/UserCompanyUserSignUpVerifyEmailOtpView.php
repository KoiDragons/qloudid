...
<script>
	    ...
	    function submitform()
		{
		   ...
		   var otpInfo=$("#otp1").val()+$("#otp2").val()+$("#otp3").val()+$("#otp4").val()+$("#otp5").val()+$("#otp6").val();
		  if(otpInfo.length>6)
		  {
			$("#errorMsg1").html('Please enter one digit in one box');
				return false;  
		  }
		  
				var send_data={};
					send_data.otp=otpInfo;
					$.ajax({
						type:"POST",
						url:"../../verifyEmailOtpDetail/<?php echo $data['request_id']; ?>",
						data:send_data,
						dataType:"text",
						...
						success: function(data1){
							...
							if(data1==0)
							{
								$("#errorMsg1").html('Wrong otp please try again');
								return false;
							}
							else
							{
...
<a class="header__logo xs-fsz25 changedText" href="../../signUpEmailInfo/<?php echo $data['domain_id']; ?>" style="
...
<div class="checkout__category changedText" style="font-weight:normal !important;">One time password</div>
...
<form class="checkout__item js-tabs-item" action="../../signUpProfessionalInfo/<?php echo $data['domain_id']; ?>/<?php echo $data['request_id']; ?>" method="POST" id="save_indexing" name="save_indexing" accept-charset="ISO-8859-1"  style="display: block;">
...
<div class="login__code" style="margin-bottom:48px; justify-content: left;">
 ...
 <div class="checkout__fieldset">
 <input type="hidden" id="email" name="email"  value="<?php echo $emailRequestReceivedInfo['email']; ?>" /> 
