	  ...
	  function submitform()
		{
			 ...
			 if($("#property_type_detail").val()=="" || $("#property_type_detail").val()==0)
			{
				...
				$("#errorMsg1").html('Please select problem type');
				return false;
			}
			...
			if($("#inclusion_type_detail").val()=="" || $("#inclusion_type_detail").val()==0)
			{
				...
				$("#errorMsg1").html('Please select problem issues');
				return false;
			}
		...
		function getSubcategoryIssue(id)
	   {
		   if(id==0)
		   {
			  $("#inclusion_type_detail").val(''); 
			  $("#inclusionInfo").addClass('hidden'); 
			  $("#inclusionData").html(''); 
		   }
		   else
		   {
			   ...
			   var send_data={};
				 send_data.subcategory_id=id;
				 $.ajax({
							type:"POST",
							url:"../../../../../../getSubcategoryIssue",
							data:send_data,
							dataType:"text",
							...
							$('#inclusionData').html('');
							$('#inclusionData').html(data1); 	  	
							  $("#inclusionInfo").removeClass('hidden'); 	  
							}
						});	
		   }
	   }
...
<div class="header__center center xxs-padrl25"><a class="header__logo xs-fsz20 changedText" href="../../../../../../professionalServicesList/<?php echo $data['domain_id']; ?>/<?php echo $data['area_id']; ?>/<?php echo $data['whom_id']; ?>/<?php echo $data['todo_id']; ?>/<?php echo $selectedProfessionalCategoryDetailInfo['md_id']; ?>" style="
...
<div class="checkout__category changedText"><?php echo $selectedProblemSubpartId['problem_subpart_name']; ?></div>
...
<form class="checkout__item js-tabs-item" action="../../../../../../addProfessionalHomeRepairRequest/<?php echo $data['catg_id']; ?>/<?php echo $data['subcatg_id']; ?>/<?php echo $data['domain_id']; ?>/<?php echo $data['area_id']; ?>/<?php echo $data['todo_id']; ?>/<?php echo $data['whom_id']; ?>" method="POST" id="save_indexing" name="save_indexing" accept-charset="ISO-8859-1" style="display: block;">
 
<div class="checkout__fieldset">
 
<div class="field"  >
<div class="field__label changedText" style="color:#525252;">What type of problem do you have?</div>
...
<select  class="nice-select select"  id="property_type_detail" name="property_type_detail" style="background:#F4F5F6; height:60px; border:none; 
    ...
    color: #777E90; border-radius: 12px;
       width:100%;" fdprocessedid="8mc52" onchange="getSubcategoryIssue(this.value);">
	   <option value="0" class="changedText">Select</option>
				<?php foreach($selectedCategoryProblemsInfo as $category => $value) 
												{
													
													
												?> 	
                           <option value="<?php echo $value['id']; ?>" class="lgtgrey2_bg changedText"><?php echo str_ireplace('&','and',$value['subcategory_name']); ?></option>
												<?php } ?>				 
                          </select>
</div>
</div>
 
<div id="inclusionInfo" class="padt10 hidden">
  
<div class="checkout__category changedText" style="color:#525252;">Inclusions</div>
<div id="inclusionData"  style="padding-bottom:32px;">
</div>
