...
class BookingInformationModel extends AppModel
{
	...
	function checkBookingCode($data)
		{
			$dbCon = AppModel::createConnection();
			$apartment_id= $this -> encrypt_decrypt('decrypt',$data['apartment_id']);
			 
			$stmt = $dbCon->prepare("select id,checked_in,checkin_code,checkin_date,checkout_date,chekin_type from hotel_checkout_detail where  room_type_id=? and (checked_in=0 or checked_in=1) and hotel_property_type=2 and checkin_code=? and is_paid=1 and reservation_confirmed=1");
			
			/* bind parameters for markers */
			$stmt->bind_param("is",$apartment_id,$_POST['ecode']);
			$stmt->execute();
			$result = $stmt->get_result();
	...
	function checkHotelBookingCode($data)
		{
			$dbCon = AppModel::createConnection();
			$apartment_id= $this -> encrypt_decrypt('decrypt',$data['hotel_id']);
			 
			$stmt = $dbCon->prepare("select id,checked_in,checkin_code,checkin_date,checkout_date,chekin_type from hotel_checkout_detail where  hotel_id=? and (checked_in=0 or checked_in=1) and hotel_property_type=1 and checkin_code=? and is_paid=1 and reservation_confirmed=1");
		...
		{
			$dbCon = AppModel::createConnection();
			$dependent_id= $this -> encrypt_decrypt('decrypt',$data['child_invitation_id']);
			$stmt = $dbCon->prepare("select id  from dependents   where  passport_number=? and id!=(select dependent_id from hotel_checkout_dependent_detail where id=?)");
		...
		function updateChildPassport($data)
		{
			$dbCon = AppModel::createConnection();
			$d_id= $this -> encrypt_decrypt('decrypt',$data['child_invitation_id']);
			...
			$stmt = $dbCon->prepare("update dependents set front_image_path=?,back_image_path=?,passport_number=?,issue_month=?,issue_year=?,expiry_month=?, expiry_year=?,is_completed=1,is_passport_updated=1 where id=(select dependent_id from hotel_checkout_dependent_detail where id=?)");
			...
			$stmt = $dbCon->prepare("update hotel_checkout_dependent_detail set is_verified=1,checked_in_dependent=1,passport_update_required=0 where id=?");
	...
	function checkinGuests($data)
		{
			$dbCon = AppModel::createConnection();
			$hotel_checkin_id= $this -> encrypt_decrypt('decrypt',$data['checkout_id']);
			$bookingInformationDetail=$this->bookingInformationDetail($data);
			if($bookingInformationDetail['checked_in']==1)
			{
			
			$adults=explode(',',$_POST['guest_adults']);
			...
			$stmt->execute();
			$result = $stmt->get_result();
			$rowAdult = $result->fetch_assoc();	
			
			if($rowAdult['invited_user_id']==null || $rowAdult['invited_user_id']=='')
			{
			$stmt = $dbCon->prepare("update  hotel_checkout_invitation_info set passport_update_required=1 where id=?");
			/* bind parameters for markers */
			$stmt->bind_param("i", $key);
			$stmt->execute();	
			}
			...
			/* bind parameters for markers */
			$stmt->bind_param("i", $rowAdult['invited_user_id']);
			$stmt->execute();
			$result = $stmt->get_result();
			...
			$stmt = $dbCon->prepare("select * from hotel_checkout_dependent_detail where hotel_checkout_id=?");
			
			/* bind parameters for markers */
			$stmt->bind_param("i", $hotel_checkin_id);
			$stmt->execute();
			$resultkey = $stmt->get_result();
			while($key = $resultkey->fetch_assoc())
			{
				
			 
			$stmt = $dbCon->prepare("select * from hotel_checkout_dependent_detail where id=?");
			...
			/* bind parameters for markers */
			$stmt->bind_param("i", $rowAdult['dependent_id']);
			$stmt->execute();
			$result = $stmt->get_result();
			$rowId = $result->fetch_assoc();
			
			 
			if($rowId['passport_number']==null || $rowId['passport_number']=='')
			{
			$stmt = $dbCon->prepare("update hotel_checkout_dependent_detail set passport_update_required=1 where id=?");
			/* bind parameters for markers */
			$stmt->bind_param("i", $key['id']);
			$stmt->execute();		
			}
			...
			{
			$stmt = $dbCon->prepare("update hotel_checkout_dependent_detail set is_verified=1,checked_in_dependent=1 where id=?");
			/* bind parameters for markers */
			$stmt->bind_param("i", $key['id']);
			$stmt->execute();	
			}
			...
			{
			if($bookingInformationDetail['hotel_property_type']==2)
			{
			$curl = curl_init();
			  curl_setopt_array($curl, array(
			  CURLOPT_URL => 'https://dstricts.com/user/index.php/DstrictsApp/checkinGuests',
			  ...
			  CURLOPT_POSTFIELDS => array('checkout_id' => $hotel_checkin_id),
			)); 
			...
			else if($bookingInformationDetail['hotel_property_type']==1)
			{
			$_POST['checkout_id']=$hotel_checkin_id;
...
<![endif]-->
			...
			$stmt->bind_param("ssisi",$cuId,$Chargeid,$room_id,$today,$hotel_checkin_id);
			$stmt->execute();	
			...
			$stmt = $dbCon->prepare("select count(id) as num from hotel_checkout_dependent_detail where hotel_checkout_id=? and passport_update_required=1");
			
			/* bind parameters for markers */
			$stmt->bind_param("i", $hotel_checkin_id);
			$stmt->execute();
			$result = $stmt->get_result();
			$rowChildren = $result->fetch_assoc();	
			
			
			$stmt = $dbCon->prepare("select count(id) as num from hotel_checkout_invitation_info where hotel_checkout_id=? and passport_update_required=1");
			...
			$stmt->close();
			$dbCon->close();
			return $rowChildren['num']+$rowAdult['num'];	
		}
		...
		function checkUserEmailInfo($data)
		{
			$dbCon = AppModel::createConnection();
			$stmt = $dbCon->prepare("select count(id) as num from user_logins where email=? and id!=?");
		...
		function checkUserPhoneInfo($data)
		{
			$dbCon = AppModel::createConnection();
			...
			/* bind parameters for markers */
			$stmt->bind_param("isi",$_POST['pass_country'],$_POST['id_number'],$_POST['user_id']);
			$stmt->execute();
			$result = $stmt->get_result();
			$row = $result->fetch_assoc();
			if($row['num']==0)
			{
			$stmt = $dbCon->prepare("select count(id) as num from user_visiting_countries where country_info=? and phone_number=? and user_login_id!=?");
		...
		function checkUserPassportInfo($data)
		{
			$dbCon = AppModel::createConnection();
			$stmt = $dbCon->prepare("select count(id) as num from user_identification where identity_number=? and user_id!=?");
			
			/* bind parameters for markers */
			$stmt->bind_param("si", $_POST['id_number'],$_POST['user_id']);
			$stmt->execute();
			$result = $stmt->get_result();
			$row = $result->fetch_assoc();
			$stmt->close();
		...
		function updateGuestPassportDetails($data)
		{
		...
		$stmt = $dbCon->prepare("update hotel_checkout_invitation_info set country_code=?,country_id=?,phone_number=?,email=?,invited_user_id=?, is_confirmed=1, pre_checked=1, passport_id=?,passport_update_required=0 where id=?");
		$stmt->bind_param("iissiii",$_POST['country_code'],$_POST['pcountry'],$_POST['p_number'],$_POST['email'], $_POST['guest_user_id'],$identificator,$invitation_id);
		$stmt->execute();
		$stmt->close(); 
		$dbCon->close();
		return 1;
		...
		function updateUserPassportDetails($data)
		{
		$dbCon = AppModel::createConnection($data);
		$invitation_id= $this -> encrypt_decrypt('decrypt',$data['invitation_id']);
		if($_POST['indexing_save']==0)
		{
		$_POST['guest_user_id']=$data['user_id'];
		$identificator=$this->addIndificator($data);	
		}
		else
		{
			$identificator=$_POST['indexing_save'];
		}
		$stmt = $dbCon->prepare("update user_logins set first_name=?,last_name=? where id=?");
		$stmt->bind_param("ssi",$_POST['first_name'], $_POST['last_name'],$data['user_id']);
		$stmt->execute();
		
		$stmt = $dbCon->prepare("update user_profiles set phone_number=? where user_logins_id=?");
		$stmt->bind_param("si",$_POST['p_number'],$data['user_id']);
		$stmt->execute();
		
		$stmt = $dbCon->prepare("update hotel_checkout_invitation_info set is_confirmed=1, pre_checked=1, passport_id=?,passport_update_required=0 where id=?");
		$stmt->bind_param("ii",$identificator,$invitation_id);
		$stmt->execute();
		$stmt->close(); 
		$dbCon->close();
		return 1;
	...
	function adultInvitationDetail($data)
		{
			$dbCon = AppModel::createConnection();
			$invitation_id= $this -> encrypt_decrypt('decrypt',$data['invitation_id']);
			$stmt = $dbCon->prepare("select * from hotel_checkout_invitation_info where id=?");
			...
			/* bind parameters for markers */
			$stmt->bind_param("ii",$row['id'], $invitation_id);
			$stmt->execute();	
			}				
			}
			else
			{
			$stmt = $dbCon->prepare("select user_logins.id from user_logins left join user_profiles on user_logins.id=user_profiles.user_logins_id where country_of_residence=? and phone_number=?");
			/* bind parameters for markers */
			...
			$row['hotel_checkout_id']=$this->encrypt_decrypt('encrypt',$row['hotel_checkout_id']);
			$stmt->close();
			$dbCon->close();
			return $row;	
		}
		...
		function hotelInformation($data)
		{
			$dbCon = AppModel::createConnection();
			$hotel_id= $this -> encrypt_decrypt('decrypt',$data['hotel_id']);
			 
			$stmt = $dbCon->prepare("select hotel_basic_information.id,hotel_name   from hotel_basic_information where id=?");
	...
	function adultsCheckedInList($data)
		...
		function dependentsCheckedInList($data)
		{
			...
			$stmt = $dbCon->prepare("select checked_in_dependent,hotel_checkout_dependent_detail.id,concat_ws(' ',first_name,last_name) as name,dependent_id from hotel_checkout_dependent_detail left join dependents on dependents.id=hotel_checkout_dependent_detail.dependent_id where hotel_checkout_id=?");
		...
		function adultsNotVerified($data)
		{
			$dbCon = AppModel::createConnection();
			$data['checkid']= $this -> encrypt_decrypt('decrypt',$data['hotel_checkin_id']);
			$org=array();
			$stmt = $dbCon->prepare("select hotel_checkout_invitation_info.id,concat_ws(' ',first_name,last_name) as name,is_confirmed,invitation_type,hotel_checkout_invitation_info.email,country_code,phone_number,pre_checked  from hotel_checkout_invitation_info left join user_logins on user_logins.id=hotel_checkout_invitation_info.invited_user_id where hotel_checkout_id=? and passport_update_required=1");
		...
		function dependentsNotverified($data)
		{
			 $dbCon = AppModel::createConnection();
			$data['checkid']= $this -> encrypt_decrypt('decrypt',$data['hotel_checkin_id']);
			$stmt = $dbCon->prepare("delete from hotel_checkout_dependent_detail where hotel_checkout_id=? and is_verified=0");
			...
			$stmt = $dbCon->prepare("select hotel_checkout_dependent_detail.id,concat_ws(' ',first_name,last_name) as name,dependent_id from hotel_checkout_dependent_detail left join dependents on dependents.id=hotel_checkout_dependent_detail.dependent_id where hotel_checkout_id=? and passport_update_required=1");
    ...
    function userSummary($data)
    {
        $dbCon = AppModel::createConnection();
          ...
          $stmt = $dbCon->prepare("select phone_number,first_name,concat_ws(' ', first_name, last_name) as name,last_name,email,passport_image,password,country_of_residence from user_logins left join user_profiles on user_logins.id=user_profiles.user_logins_id where user_logins.id = ?");
        /* bind parameters for markers */
		$cont=1;
       $stmt->bind_param("i", $data['user_id']);
        $stmt->execute();
	...
	function userPassport($data)
    {
        $dbCon = AppModel::createConnection();
       
		
        $stmt = $dbCon->prepare("select * from user_identification where user_id = ? and identification_type=1");
        /* bind parameters for markers */
		$cont=1;
        $stmt->bind_param("i", $data['user_id']);
        $stmt->execute();
	 ...
	 function reservationConfermationRequiredList($data)
		{
			$dbCon = AppModel::createConnection();
			$date=strtotime(date('Y-m-d'));
		...
		function invoiceAddressesCompany($data)
		{
		$dbCon = AppModel::createConnection();	
		$checkout_id=$this->encrypt_decrypt("decrypt",$data['checkout_id']);
		$stmt = $dbCon->prepare("select count(qloud_checkout_id) as num from hotel_checkuout_confirmation_info where qloud_checkout_id=?");
		...
		if($row['num']==0)
		{
		$stmt = $dbCon->prepare("insert into hotel_checkuout_confirmation_info (qloud_checkout_id) values(?)");
		...
		{
		$stmt = $dbCon->prepare("update hotel_checkuout_confirmation_info set is_company=0,buyer_id=0,card_id=0,address_id=0 where qloud_checkout_id=?");
		...
		$stmt = $dbCon->prepare("update hotel_checkuout_confirmation_info set card_id=? where qloud_checkout_id=?");
			
		/* bind parameters for markers */
		$stmt->bind_param("ii",$card_id, $checkout_id);
		$stmt->execute();		
		$stmt->close();
		$dbCon->close();
		return 1;	
		...
		$dbCon = AppModel::createConnection();	
		$checkout_id=$this->encrypt_decrypt("decrypt",$data['checkout_id']);
		$address_id=$this->encrypt_decrypt("decrypt",$data['address_id']);
		
		$stmt = $dbCon->prepare("update hotel_checkuout_confirmation_info set is_company=0,buyer_id=?,address_id=? where qloud_checkout_id=?");
			
		/* bind parameters for markers */
		$stmt->bind_param("iii",$data['user_id'],$address_id, $checkout_id);
		$stmt->execute();		
		$stmt->close();
		$dbCon->close();
		return 1;	
		...
		$checkout_id=$this->encrypt_decrypt("decrypt",$data['checkout_id']);
		$address_id=$this->encrypt_decrypt("decrypt",$data['address_id']);
		$stmt = $dbCon->prepare("select company_id from company_profiles where id=?");
		/* bind parameters for markers */
		$stmt->bind_param("i", $address_id);
		$stmt->execute();
		$result = $stmt->get_result();
		$row = $result->fetch_assoc();
		$stmt = $dbCon->prepare("update hotel_checkuout_confirmation_info set is_company=1,buyer_id=?,address_id=? where qloud_checkout_id=?");
			
		/* bind parameters for markers */
		$stmt->bind_param("iii",$row['company_id'],$address_id, $checkout_id);
		$stmt->execute();		
		$stmt->close();
		$dbCon->close();
		return 1;	
		...
		function invoiceAddressesUser($data)
		{
		$dbCon = AppModel::createConnection();	
		$checkout_id=$this->encrypt_decrypt("decrypt",$data['checkout_id']);
		$stmt = $dbCon->prepare("select count(qloud_checkout_id) as num from hotel_checkuout_confirmation_info where qloud_checkout_id=?");
		...
		function cardsList($data)
		{
			...
			$dbCon = AppModel::createConnection();
			$checkout_id=$this->encrypt_decrypt("decrypt",$data['checkout_id']);
			$stmt = $dbCon->prepare("select is_company,buyer_id from hotel_checkuout_confirmation_info where qloud_checkout_id=?");
		 ...
		 else
		 {
				...
				$stmt->bind_param("ii", $data['user_id'],$row['buyer_id']);
				$stmt->execute();
				$result = $stmt->get_result();
				while($row = $result->fetch_assoc())
				{
		...
		function verificationInfomation($data)
		{
			
			$dbCon = AppModel::createConnection();
			$checkout_id=$this->encrypt_decrypt("decrypt",$data['checkout_id']);
			$stmt = $dbCon->prepare("select is_company,buyer_id,address_id,card_id from hotel_checkuout_confirmation_info where qloud_checkout_id=?");
			...
			if($row['is_company']==0)
			{
			$stmt = $dbCon->prepare("select id,name_on_card,card_number,card_cvv,exp_month,exp_year,card_type from  user_cards where `id`=?");
			$stmt->bind_param("i", $row['card_id']);
			$stmt->execute();
			$result = $stmt->get_result();
			$rowCard = $result->fetch_assoc();
			$row['name_on_card']=html_entity_decode($rowCard['name_on_card'],ENT_NOQUOTES, 'UTF-8');
			$row['card_number2']='**** **** **** '.substr($rowCard['card_number'],-4);
			...
			$row['name_on_house']=html_entity_decode($rowAddress['name'],ENT_NOQUOTES, 'UTF-8');
			$row['invoice_address']=html_entity_decode($rowAddress['invoice_address'],ENT_NOQUOTES, 'UTF-8');
			$row['invoice_city']=html_entity_decode($rowAddress['invoice_city'],ENT_NOQUOTES, 'UTF-8');
			$row['invoice_zip']=html_entity_decode($rowAddress['invoice_zipcode'],ENT_NOQUOTES, 'UTF-8');
			$row['invoice_country']=html_entity_decode($rowAddress['country_name'],ENT_NOQUOTES, 'UTF-8');
			$row['invoice_port_number']=html_entity_decode($rowAddress['invoice_port_number'],ENT_NOQUOTES, 'UTF-8'); 	
			 
			}
		 else
		 {
				$stmt = $dbCon->prepare("select id,name_on_card,card_number,card_cvv,exp_month,exp_year,card_type from  company_cards where `id`=?");
				$stmt->bind_param("i", $row['card_id']);
				$stmt->execute();
				$result = $stmt->get_result();
				$rowCard = $result->fetch_assoc();
				$row['name_on_card']=html_entity_decode($rowCard['name_on_card'],ENT_NOQUOTES, 'UTF-8');
				$row['card_number2']='**** **** **** '.substr($rowCard['card_number'],-4);
				$stmt = $dbCon->prepare("select company_profiles.company_id,country_name,company_name,company_profiles.id,si_address,si_city,si_zip,si_country,i_port_number from company_profiles left join companies on companies.id=company_profiles.company_id left join phone_country_code on phone_country_code.id=companies.country_id where company_profiles.id=?");
				/* bind parameters for markers */
				$stmt->bind_param("i", $row['address_id']);
				$stmt->execute();
				$result = $stmt->get_result();
				$rowAddress = $result->fetch_assoc();
				$row['name_on_house']=html_entity_decode($rowAddress['company_name'],ENT_NOQUOTES, 'UTF-8');
				$row['invoice_address']=html_entity_decode($rowAddress['si_address'],ENT_NOQUOTES, 'UTF-8');
				$row['invoice_city']=html_entity_decode($rowAddress['si_city'],ENT_NOQUOTES, 'UTF-8');
				$row['invoice_zip']=html_entity_decode($rowAddress['si_zip'],ENT_NOQUOTES, 'UTF-8');
				$row['invoice_country']=html_entity_decode($rowAddress['country_name'],ENT_NOQUOTES, 'UTF-8');
				$row['invoice_port_number']=html_entity_decode($rowAddress['i_port_number'],ENT_NOQUOTES, 'UTF-8'); 
		}
			$stmt->close();
			$dbCon->close();
			return $row;
		...
		function reservationVerificationInfomation($data)
		{
			
			$dbCon = AppModel::createConnection();
			$checkout_id=$this->encrypt_decrypt("decrypt",$data['checkout_id']);
			$stmt = $dbCon->prepare("select is_buyer_company as is_company,buyer_id,delivery_address_id as address_id,card_id from hotel_checkout_detail where id=?");
    ...
    function confirmApartmentReservation($data)
		...
		$data['invoice_address_id']=$row['address_id'];
		$data['property_id']=$checkout_id;
		if($row['is_company']==1)
		{
		$data['is_company']=$row['buyer_id'];	
		}
		else
		{
		$data['is_company']=0;	
		...
		function confirmBookingReservation($data)
		{
		$dbCon = AppModel::createConnection();
		$checkout_id=$this->encrypt_decrypt("decrypt",$data['checkout_id']);
		$data['property_id']=$checkout_id;
			...
			$stmt = $dbCon->prepare("update hotel_checkout_detail set reservation_confirmed=1,checkin_code=? where id=?");
			$stmt->bind_param("ii",$code, $data['property_id']);
			$stmt->execute();
			  ...
			  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
			  CURLOPT_CUSTOMREQUEST => 'POST',
			  CURLOPT_POSTFIELDS => array('checkin_code'=>$code,'is_paid' =>1,'card_id' => $row['card_id'],'address_id' => $row['delivery_address_id'],'cuId' => $row['cust_id'],'Chargeid' => $row['transection_id'],'checkout_id' => $data['property_id'],'is_company'=>$row['is_buyer_company'],'buyer_id'=>$row['buyer_id']),
			)); 
			$response = curl_exec($curl);
			curl_close($curl);
