...
(function(factory){
    ...
    Tokenize2.DEFAULTS = {
		container_class: 'mar0 pad0',
		token_class: 'diblock pos_rel marb5 marr5 padtb5 padr20 padl8 brd brdclr_428f8c brdrad2 bg_428f8c_05 bg_428f8c_2_h valm txt_428f8c',
		token_text_class: 'valm',
		token_dismiss_class: 'dblock pos_abs pos_cenY right0 pad5 valm txt_a9b9d8 curp',
		token_dismiss_html: 'x',
		token_search_class: 'diblock marb5 pad0 valm',
		token_field_class: 'wi_100 hei_32p mar0 pad0 nobrd trans_bg',
		
		dropdown_class: 'maxwi_300p dblock pos_abs',
		menu_class: 'wi_100 dblock minhei_10p vis_vis opa1 mar0 pad0 brd white_bg',
		item_class: 'dblock mar0 pad0',
		link_class: 'dblock pos_rel padtb5 padrl15 bg_428f8c_2_h lgn_hight_24 txt_428f8c curp',
		hightlight_class: 'bold',
		empty_class: 'dblock pos_rel padtb5 padrl15 lgn_hight_24 txt_428f8c',
    ...
    Tokenize2.prototype.init = function(){
        ...
        this.id = this.guid();
        //this.element.hide();
		this.element.addClass('wi_100 hei_100 opa0 pos_abs zi1 top0 left0');
        ...
        this.searchContainer = $('<li class="token-search ' + this.options.token_search_class + '" />');
        this.input = $('<input autocomplete="off" class="' + this.options.token_field_class + '" />')
        ...
        this.tokensContainer = $('<ul class="tokens-container form-control ' + this.options.container_class + '" />')
        ...
        this.container = $('<div class="tokenize pos_rel zi2" />').attr('id', this.id);
    ...
    Tokenize2.prototype.tokenAdd = function(value, text, force){
        ...
        $('<li class="token ' + this.options.token_class + '" />')
            .attr('data-value', value)
            .append('<span class="' + this.options.token_text_class + '">' + text + '</span>')
            .prepend($('<a class="dismiss ' + this.options.token_dismiss_class + '">' + this.options.token_dismiss_html + '</a>').on('mousedown touchstart', {}, $.proxy(function(e){
                e.preventDefault();
    ...
    Tokenize2.prototype.dropdownShow = function(){
        ...
        if(!this.isDropdownOpen()){
            ...
            this.dropdown = $('<div class="tokenize-dropdown dropdown ' + this.options.dropdown_class + '"><ul class="dropdown-menu ' + this.options.menu_class + '" /></div>').attr('data-related', this.id);
        ...
        if(items && items.length > 0){
            ...
            if(this.options.displayNoResultsMessage){
                ...
                this.dropdown.find('.dropdown-menu').append(
                    $('<li class="dropdown-item ' + this.options.item_class + ' locked" />').html('<span class="' + this.options.empty_class + '">' + this.options.noResultsMessageText.replace('%s', this.input.val()) + '</span>')
                );
            } else {
                this.trigger('tokenize:dropdown:hide');
            }
    ...
    Tokenize2.prototype.dropdownAddItem = function(item){
        ...
        if(this.isDropdownOpen()){
            var $li = $('<li class="dropdown-item ' + this.options.item_class + '" />').html(this.dropdownItemFormat(item))
    ...
    Tokenize2.prototype.dropdownItemFormat = function(item){
        ...
        if(item.hasOwnProperty('text')){
            ...
            if(this.options.searchHighlight){
                ...
                $display = item.text.replace($regex, '<span class="tokenize-highlight ' + this.options.hightlight_class + '">$1</span>');
            } else {
                $display = item.text;
            }

            return $('<a class="' + this.options.link_class + '" />').html($display).attr({
