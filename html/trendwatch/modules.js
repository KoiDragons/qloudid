		...
		$('.slider-range').each(function(){
			...
			if($this.data('capture')){
				...
				slide_options['change'] = function(event, ui){
					...
					$capture.val(ui_val);
					$('#oil_numbers').val(ui_val);
					var t_price=$('#oil_price').val()*$('#oil_numbers').val();
					$('#totalPrice').html(t_price);
					$('#totalValue').removeClass('hidden');
					$('#selectSlider').addClass('hidden');
					$('.slider-value').html('Quantity: '+ui_val+'<span class="black_txt fsz16 fright hidden-xs nobold darkgrey_txt"> Change</span><span class="black_txt fsz16 fright visible-xs nobold darkgrey_txt padr20"> <li class="far fa-edit"></li></span>');
					var capture_trigger = $this.data('capture-trigger');
					if(capture_trigger){
						$capture.trigger(capture_trigger);
					}
...
$(document).ready(function() {
	...
	$body.on('click', '.toggle-btn', function(){
		...
		if($this.data('counter') || $this.data('counter-ids')){
			var $counter = $this.data('$counter');
		
			if(!$counter){
				$counter = $this.data('counter-ids') ? $($this.data('counter-ids')) : $('#' + $this.data('counter'));
