...
$(document).ready(function() {
			...
			if($option.attr('data-value') === 'true'){
				...
				if($target[0]){
					$target
						...
						$.get("Availability/availabilityUpdate",function(data3,status){
							   
								  if(data3)
									{
										window.location ="http://www.qmatchup.com/beta/profile-user-cv.php";
									}
									else
									{
	...
	$body.on('click', '.editable', function(){
		...
		if($this.hasClass('edit-text')){
			var $edit_input = $('<input type="text" id="cid" />');
			//console.log("hello");
			$edit_input
		...
		if($this.hasClass('edit-address')){
			...
			if(options){
				...
				send_data.company_id=document.getElementById('company_id').value;
				send_data.cid=$select.val();
			if($this.hasClass('jain4'))
			{
			send_data.uid=1;	
			}
			else if($this.hasClass('jain5'))
			{
			send_data.uid=2;	
			}
		
				$.ajax({
			url: '../../Location/updateSize',
		type: 'POST',
		dataType: 'json',
		data: send_data
		})
		...
		.always(function(){});
			...
			if($this.hasClass('jain2'))
			{
			send_data.uid=1;	
			}
			else if($this.hasClass('jain3'))
			{
			send_data.uid=2;	
			}
		
				$.ajax({
			url: '../../Location/updateDate',
		type: 'POST',
		dataType: 'json',
		data: send_data
		})
	...
	$body.on('blur', '.post-edit-text', function(){
		...
		else if($this.hasClass('jain11'))
			{
			send_data.uid=3;	
			}
			else if($this.hasClass('jain12'))
			{
			send_data.uid=4;	
			}
				$.ajax({
			url: '../../Location/updateCid',
		type: 'POST',
		dataType: 'json',
		data: send_data
		})
	...
	/*$body.on('blur', '.post-edit-datepicker', function(){
		var $this = $(this),
			$editable = $this.data('$el'),
			val = $this.val();
		var send_data = {};
