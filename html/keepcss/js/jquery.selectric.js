		...
		init = function(element, options) {
			...
			function _populate() {
				var $options = $original.children(),
					dcols = $original.attr('data-columns'),
					_$li = '',
					init_call = true,
					item_class = 0,
				...
				if(dcols){
					_$li = '<div class="selectric_table"><div class="selectric_cell"><ul>';
				}
				else{
					_$li = '<ul>';
				}
				...
				if (optionsLength = $options.length) {
					...
					$options.each(function(i) {
						...
						var $elm = $(this),
							...
							elmIcon = $elm.attr('data-icon'),
							elmIndexIcon = $elm.attr('data-index-icon'),
							elmIndexClass = $elm.attr('data-index-class'),
							dtitle = $elm.attr('data-title'),
							dfiller = $elm.attr('data-filler'),
							ddefault = $elm.attr('data-default');
						...
						if(dcols){
							if(dtitle){
								if(!init_call){
									_$li += '</ul></div><div class="selectric_cell"><ul>';
								}
								init_call = false;
								item_class++;
							}
							_$li += '<li data-index="' + c_index + '" data-value="' + $elm.val() + '" class="' + (dtitle ? 'title ' : '') + (elmIcon ? 'hasIcon ' : '') + (ddefault ? 'default ' : '') + (dfiller ? 'filler ' : '') + (i == currValue ? selectStr : '') + (i == optionsLength - 1 ? ' last' : '') + (selectDisabled ? ' disabled' : '') + '">' + (elmIndexIcon ? '<span class="' + elmIndexClass + '">' + elmIndexIcon + '</span>' : '') + (elmIcon ? '<img src="' + elmIcon + '" />' : '') + optionText + '</li>';
					...
					$items
						.addClass('disabled-items disabled-' + item_class)
						.html(_$li);
