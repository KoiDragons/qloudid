...
$(document).ready(function() {
	...
	if($.fn.tooltip){
		...
		$('.tooltip').each(function(){
			...
			var def_settings = $this.data('tooltip-settings') ? $this.data('tooltip-settings').split(',') : false;
			if(def_settings){
	...
	if($.fn.slider){
		...
		$('.slider-range').each(function(){
			...
			if($this.data('capture')){
				...
				slide_options['slide'] = function(event, ui){
					var $this_control = $(this),
						$capture = $this_control.data('$capture'),
						fixed_min = $this_control.data('fixed-min'),
						ui_val = ui.value;
					if(fixed_min){
						if(ui.value <= $this_control.data('min')){
							ui_val = fixed_min;
						}
					}
					$capture.val(ui_val);
					
					var capture_trigger = $this.data('capture-trigger');
					if(capture_trigger){
						$capture.trigger(capture_trigger);
					}
				};
			...
			slide_options['step'] = $this.data('step') ? $this.data('step') : 1;
	...
	*/
	...
	/*
		...
		data-type [string] = classes toggle type (TODO)
...
$(document).ready(function() {
	...
	$body.on('click', '.toggle-btn', function(){
		...
		if($target[0]){
			$target.each(function(){
				...
				if($ctar.is(":visible")){
					...
					if($this.data('hide')){
						if(id == 'base'){
							$hides = $this.closest('.base').siblings('.base').find('.toggle_content');
						}
		...
		var counter = $this.data('counter'),
			counters = $this.data('counter-ids');
		if(counter || counters){
			var $counter = $this.data('$counter');
		
			if(!$counter){
				$counter = counters ? $(counters) : $('#' + counter);
				$this.data('$counter', $counter);
			}
			if($counter[0]){
				$counter.each(function(){
					var $ccount = $(this);
					if($ccount.is(":visible")){
						$ccount
							.stop()
							.slideUp();
					}
	...
	$body.on('click', '.expander-toggler', function(){
		...
		if($body.data('expander-toggler-init') != true){
			$body
				.on('click', function(){
					$body.find('.expander-toggler.autohide').each(function(){
						let $this = $(this),
							$target = $this.data('$target');
						if($target && $target[0] && $target.is(':visible')){
				...
				.data('expander-toggler-init', true);
		}
		...
		if($target[0]){
			...
			if($target.is(':visible')){
				...
				if(hide){
					var $hides_base = $($this.closest($this.data('hide-base')));
					if($hides_base[0]){
						var $hides = $hides_base.find('.expander-toggler.active');
						for(var h = 0, hL = $hides.length; h < hL; h++){
							var $hide = $hides.eq(h);
							
							if($this.data('target') !== $hide.data('target')){
								var $hide_target = $hide.data('$target');
								
								if(!$hide_target){
									var hide_target = $hide.data('target');
									if(!hide_target){
										hide_target = $hide.attr('href');
									}
									
									var hide_base = $hide.data('base');
									if(hide_base){
										$target = $hide.closest(hide_base).find(hide_target);
									}
									else{
								...
								if($hide_target[0]){
									$hide
										.data('$target', $hide_target)
										.removeClass('active');
									clearTimeout($hide_target.data('tm'));
									$hide_target
										.stop()
										.slideUp(300);
								}
	...
	if($.fn.stick_in_parent){
		$('.sticky').each(function(){
			let $this = $(this),
				offset_top = $this.data('offset') ? $this.data('offset') : 0;
