				...
				if (optionsLength = $options.length) {
					...
					$options.each(function(i) {
						...
						if(dcols){
							...
							_$li += '<li data-index="' + c_index + '" data-value="' + $elm.val() + '" class="' + (dtitle ? 'title ' : '') + (elmIcon ? 'hasIcon ' : '') + (ddefault ? 'default ' : '') + (dfiller ? 'filler ' : '') + (i == currValue ? selectStr : '') + (i == optionsLength - 1 ? ' last' : '') + (selectDisabled ? ' disabled' : '') + '">' + (elmIcon ? '<img src="' + elmIcon + '" />' : '') + optionText + '</li>';
