		...
		function checkEmail(id) {
			...
			function submitform()
			{
				...
				$("#errorMsg1").html('');
				 if($("#total_paid").val()==0)
				{
				$("#errorMsg1").html('Paid amount can not be 0');	
				return false;
				}
				...
				send_data.id=$("#deposit_code").val();
				$.ajax({
					type:"POST",
					url:"../checkDepositCode/<?php echo $data['cid']; ?>",
					data:send_data,
					dataType:"text",
					contentType: "application/x-www-form-urlencoded;charset=utf-8",
					success: function(data1){
						if(data1==2)
						{
							$("#errorMsg1").html('Invoice dont belong to your company. Please use other code.');
						}
						else if(data1==0)
						{
							$("#errorMsg1").html('Transection code already used. Please use other code.');
						}
						else
						{
							document.getElementById('save_indexing1').submit();	
...
<a class="header__logo xs-fsz25 changedText" href="../listInvoice/<?php echo $data['cid']; ?>" style="
...
<form action="../updatePayment/<?php echo $data['cid']; ?>" method="POST" id="save_indexing1" name="save_indexing1" accept-charset="ISO-8859-1">
...
<div class="field">
<div class="field__label changedText" style="color:#787f91;">Transection code</div>
<div class="field__wrap">
<input class="field__input"  name="deposit_code" id="deposit_code" type="number"   style=" color:grey;"  >
</div>
</div>
</div>
</div>
