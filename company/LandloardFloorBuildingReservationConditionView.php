   ...
   function submitForm()
			{
				...
				var installment_fee_detail=0;
				for(i=1;i<=$('#total_installment_allowed').val();i++)
				{
					installment_fee_detail=parseInt(installment_fee_detail)+parseInt($('#installment'+i).val());
				}
				  
				if((parseInt($("#random_reservation_fee").val())+parseInt($("#possetion_time_fee").val())+parseInt(installment_fee_detail))!=100)
				{
					 
					$("#error_msg_form").html('Total fee to be paid must be equal to 100%');
					return false;
				}
...
<a class="header__logo xs-fsz20 changedText " href="../../buildingAddinsInfo/<?php echo $data['cid']; ?>/<?php echo $data['bid']; ?>" style="
...
<div class="travel__subtitle changedText " style="
            ...
            <div class="travel__content changedText xs-fsz27" style=" line-height: 1.4;
                  color: #787f91;
				   line-height: 1.4;
                  font-size: 27px;
                  ">Complete the fields below to update your reservation conditions.</div>
...
<form class="checkout__item js-tabs-item" action="../../addReservationPriceInformation/<?php echo $data['cid']; ?>/<?php echo $data['bid']; ?>" method="POST" id="save_indexing" name="save_indexing" accept-charset="ISO-8859-1" style="display: block;">
 ...
 <div class=" mart40" id="propertyDetailStatusInfo">
				   ...
				   <div class="field margin-bottom24 " id="RandomInfo">
                            <div class="field__label  changedText" style="color:#787f91;">Reservation fee</div>
                            <div class="field__wrap">
							 <select class="" id="random_reservation_fee" name="random_reservation_fee" style="height: 48px;
						  ...
						  <div class="field margin-bottom24 ">
                            <div class="field__label  changedText" style="color:#787f91;">Total weeks for reservation period</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="total_weeks" id="total_weeks" value="1" placeholder="e. g. &quot;Spectacular views of Queenstown&quot;" style=" color:grey;"  >
                            </div>
                          </div>
						   ...
						   <script>
									function updateReservationInfo(link)
										   {
											  body = $('body');
											   if($('#reservation_price_pay_back').val()==1)
											   {
												    $('#bodyData').addClass('dark');
													 
												     $(link).attr('checked',''); 
													$('#reservation_price_pay_back').val(0);	
											   ...
											   else
												   {
													 ...
													 $('#reservation_price_pay_back').val(1);
 ...
 <input type="hidden" id="reservation_price_pay_back" name="reservation_price_pay_back" value="1"/>
				   <div class="setting__notification">
                      <div class="setting__main">
                        <div class="setting__subtitle">Pay back</div>
                        <label class="switch">
                          <input class="switch__input" type="checkbox" onclick='updateReservationInfo(this);'  checked /><span class="switch__inner"><span class="switch__box"></span></span>
                        </label>
                      </div>
                      <div class="setting__content">Do you pay back reservation fee if not purchased?</div>
                    </div>
						  
						  
						   <div class="field margin-bottom24">
                            <div class="field__label  changedText" style="color:#787f91;">Payment on key receiving</div>
                            <div class="field__wrap">
							 <select class="" id="possetion_time_fee" name="possetion_time_fee" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								  <?php for($i=1; $i<=70; $i++) { ?>
						  ...
						  <script>
						  var installment_option='<?php echo $options; ?>';
						  function updateInstallemtsDetail(id)
						  {
							   var installmet='';
							   var resale='';
							  for(i=1;i<=id;i++)
							  {
		installmet=installmet+'<div class="field margin-bottom24"><div class="field__label  changedText" style="color:#787f91;">How much to be paid</div><div class="field__wrap"><select class="" id="installment'+i+'" name="installment'+i+'" style="height: 48px;padding: 0 14px; border-color: #353945;color: grey; border-radius: 12px;background: transparent; width:100%;"   fdprocessedid="zc7mun">'+installment_option+'</select> </div></div>'; 
		resale=resale+'<option value="'+i+'" >'+i+'</option>';
							  }
							  $('#Installments').html(installmet);
							   $('#resale_after_installments').html(resale);
							   $('#resale_after_installments').val(1);
						  }
						 
						  </script>
						  
						    <div class="field margin-bottom24">
                            <div class="field__label  changedText" style="color:#787f91;">How many installments allowed</div>
                            <div class="field__wrap">
							 <select class="" id="total_installment_allowed" name="total_installment_allowed" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun" onchange="updateInstallemtsDetail(this.value);">
								  <?php for($i=1; $i<=10; $i++) { ?>
											<option value="<?php echo $i; ?>" ><?php echo $i; ?></option>
								  <?php } ?>	 
								  </select>
						  ...
						  <div id="Installments">
						  <div class="field margin-bottom24">
                            <div class="field__label  changedText" style="color:#787f91;">How much to be paid on installment</div>
                            <div class="field__wrap">
							 <select class="" id="installment1" name="installment1" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								  <?php echo $options; ?>	 
						  ...
						  <script>
									function updateResaleInfo(link)
										   {
											  body = $('body');
											   if($('#resale_allowed').val()==1)
											   {
												    $('#bodyData').addClass('dark');
													 
												    $('#resaleInfo').addClass('hidden');
												     $(link).attr('checked',''); 
													$('#resale_allowed').val(0);	
													 
											   }
											   else
												   {
													 
												   $('#resaleInfo').removeClass('hidden');
												     $('#bodyData').addClass('dark');
												     $(link).attr('checked','checked');  
													 $('#resale_allowed').val(1);
 ...
 <input type="hidden" id="resale_allowed" name="resale_allowed" value="1"/>
				   <div class="setting__notification">
                      <div class="setting__main">
                        <div class="setting__subtitle">Resale</div>
                        <label class="switch">
                          <input class="switch__input" type="checkbox" onclick='updateResaleInfo(this);'  checked /><span class="switch__inner"><span class="switch__box"></span></span>
                        </label>
                      </div>
                      <div class="setting__content">Do you allow resale of property?</div>
                    </div>
                  
				   <div class="field margin-bottom24 " id="resaleInfo">
                            <div class="field__label  changedText" style="color:#787f91;">After how many installements</div>
                            <div class="field__wrap">
							 <select class="" id="resale_after_installments" name="resale_after_installments" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
						   ...
						   <div class="field margin-bottom24">
                            <div class="field__label  changedText" style="color:#787f91;">Broker fee after each installment</div>
                            <div class="field__wrap">
							 <select class="" id="broker_fee_payment" name="broker_fee_payment" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								  <?php for($i=1; $i<=100; $i++) { ?>
 ...
 <input type="hidden" id="discount_allowed" name="discount_allowed" value="1"/>
				   <div class="setting__notification">
                      ...
                      <div class="setting__content">If there is a discunt on full payment?</div>
                    </div>
