	...
	class BookingServicesModel extends AppModel
	{
		 ...
		 function editWorkingTimeInfo($data)
		{
			...
			if($_POST['food2']==1)
			{
				 ...
				 $night_booking_available=1;
			 
				 $night_time_start=$_POST['night_time_start'];
				 $night_time_close=$_POST['night_time_close'];
			 
			}
			else
			{
				$night_booking_available=0;
				$night_time_start='19:00';
				$night_time_close='05:00';
			}
			 ...
			 if($_POST['food3']==1)
			{
				 ...
				 $lunch_close_time=$_POST['lunch_close_time'];
			...
			else
			{
				...
				$lunch_close_time='14:00';
			}
			 
			 
			
			  $stmt = $dbCon->prepare("update company_venue_opening_closing_timings set  mon_open=?, mon_open_time=?, mon_close_time=?, tue_open=?, tue_open_time=?, tue_close_time=?, wed_open=?, wed_open_time=?, wed_close_time=?, thu_open=?, thu_open_time=?, thu_close_time=?, fri_open=?, fri_open_time=?, fri_close_time=?, sat_open=?, sat_open_time=?, sat_close_time=?, sun_open=?, sun_open_time=?, sun_close_time=?,lunch_available=?,lunch_start_time=?,lunch_close_time=?,night_booking_available=?,night_time_start=?,night_time_close=?  where venue_id=? ");
			$stmt->bind_param("issississississississississi", $mon_open, $mon_open_time, $mon_close_time, $tue_open, $tue_open_time, $tue_close_time, $wed_open, $wed_open_time, $wed_close_time, $thu_open, $thu_open_time, $thu_close_time, $fri_open, $fri_open_time, $fri_close_time, $sat_open, $sat_open_time, $sat_close_time, $sun_open, $sun_open_time, $sun_close_time ,$lunch_available,$lunch_start_time,$lunch_close_time,$night_booking_available,$night_time_start,$night_time_close,$venue_id);
		...
		{ 
			...
			$dbCon = AppModel::createConnection();
			$aid=$this->encrypt_decrypt('decrypt',$data['venue_id']);
			$orgDate=array();
			...
			{
			$c=strtotime($i.'-'.$a.'-'.$b);	
			$stmt = $dbCon->prepare("select count(blocked_date) as num from company_venue_blocked_dates where venue_id=? and blocked_date=?");
...
return $org;
		...
		{ 
			...
			$_POST['selected_dates']=substr($_POST['selected_dates'],0,-1);
			 ...
			 foreach($a as $key)
			 { 
			$stmt = $dbCon->prepare("delete from company_venue_blocked_dates where blocked_date=? and venue_id=?");
		...
		{ 
			...
			$_POST['selected_dates']=substr($_POST['selected_dates'],0,-1);
			 ...
			 foreach($a as $key)
			 {
			$stmt = $dbCon->prepare("insert into company_venue_blocked_dates (blocked_date,venue_id) values(?, ?)");
   ...
   <div style="display: flex;">
     ...
     <button color="#444444" data-testid="icon-button--right" label="next month" class="merlin-button css-6t4pca" aria-label="next month" onclick="updateNextCalender(1);">
		...
		function VenueBookingDetail($data)
		{
			$dbCon = AppModel::createConnection();
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			$stmt = $dbCon->prepare("select  company_venue.id,venue_name,visiting_address from company_venue left join property_location on property_location.id=company_venue.location_id where company_venue.company_id=?  order by company_venue.id desc");
		 ...
		 function moreResturantDetail($data)
		{
			...
			while($row = $result->fetch_assoc())
			{
				...
				$org=$org.'<a href="../serveBookingInfo/'.$data['cid'].'/'.$enc.'"><div class="column_m container  marb5  fsz14 dark_grey_txt brd">
												<div class="green_bg_00ff9996   bg_fffbcc_a brdrad5">
										<div class="container padtb15  brdrad1 fsz14 dark_grey_txt padrl10">
											<div class="passport signin_bx dflex fxwrap_w xs-alit_c pos_rel tall">
												...
												<div class="wi_100 xs-wi_100 xs-order_3 xs-padt0 fsz12">
													...
													<span class="edit-text   jain2 dblock   brdclr_lgtgrey2 fsz14  black_txt   ">'.$row['visiting_address'].'</span>
													 </div>
													 
												 <div class="fright wi_65p marr15 "> <div class="wi_65p   minhei_65p   fxshrink_0 dflex alit_c justc_c uppercase fsz26 xs-fsz20 brdrad1000 yellow_bg_a  green_bg_81ceaa " style="background-repeat: no-repeat;
...
<![endif]-->
		...
		function dishesList($data)
		{
			...
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			$stmt = $dbCon->prepare("select id,dish_name,dish_detail,dish_price,dish_image,is_active from dishes_detail_information where company_id=? and dish_type!=1 and is_physical=2 and is_bookable=1");
	 ...
	 function getPermissionDetail($data)
		...
		$stmt = $dbCon->prepare("select id from manage_apps_permission where country_id=(select country_id from companies where id=?)  and app_id=49");
			/* bind parameters for markers */
			$cont=1;
			$stmt->bind_param("i", $company_id);
			$stmt->execute();
		...
		function appId()
		{
			...
			return $this -> encrypt_decrypt('encrypt',49);
		}
		function selectIcon($data)
		{
			$dbCon = AppModel::createConnection();
			
			$stmt = $dbCon->prepare("select app_bg,app_name,app_icon  from apps_detail where id=49");
...
$i++;									
		 ...
		 function moreReservationList($data)
		{
			...
			while($row = $result->fetch_assoc())
			{
				...
				$org=$org.'<a href="../../selectTables/'.$data['cid'].'/'.$data['rid'].'/'.$enc.'"> <div class="column_m container  marb5  fsz14 dark_grey_txt brd">
												<div class="green_bg_00ff9996   bg_fffbcc_a brdrad5">
										<div class="container padtb15  brdrad1 fsz14 dark_grey_txt padrl10">
											<div class="passport signin_bx dflex fxwrap_w xs-alit_c pos_rel tall">
												...
												<div class="wi_100 xs-wi_100 xs-order_3 xs-padt0 fsz12">
													...
													<span class="edit-text   jain2 dblock   brdclr_lgtgrey2 fsz14  black_txt   ">Company size - '.$row['company_size'].'</span>
													 </div>
		...
		function companyDetail($data)
		{
			...
			$stmt = $dbCon->prepare("select booking_start_time,booking_end_time,gap_between_bookings,cancellation_period,cancellation_period_time,companies.country_id,companies.id,company_name,cid,founded,industry,profile_pic,address,company_profiles.city,zip,company_profiles.country,delivery_address,d_zip,d_city,d_country,company_profiles.phone,company_email,vat,company_profiles.phone_country,si_address,si_city,si_zip,si_country,sd_address,sd_city,sd_zip,sd_country,bankgiro_med,bankgiro_utan,iban,bic,bank,f_tax from companies left join company_profiles on companies.id=company_profiles.company_id  where companies.id=?");
		...
		function updateServiceBookingRules($data)
		{
			$dbCon = AppModel::createConnection();
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			
			 
			$stmt = $dbCon->prepare("update company_profiles set booking_start_time=?,booking_end_time=?,gap_between_bookings=?,cancellation_period=?,cancellation_period_time=? where company_id=?");
			
			/* bind parameters for markers */
			$stmt->bind_param("iiiiii",$_POST['booking_start_time'],$_POST['booking_end_time'],$_POST['gap_between_bookings'],$_POST['cancellation_period'],$_POST['cancellation_period_time'], $company_id);
			$stmt->execute();
		...
		{
			$dbCon = AppModel::createConnection();
			$dish_id= $this -> encrypt_decrypt('decrypt',$data['did']);
			$stmt = $dbCon->prepare("select is_active from dishes_detail_information  where id=?");
			...
			$stmt = $dbCon->prepare("update dishes_detail_information set `is_active`=?  where id=?");
			
			/* bind parameters for markers */
			$stmt->bind_param("ii",$is_active,$dish_id);
			$stmt->execute();
		  ...
		  function moreEmployeesDetail($data)
		{
			...
			$a=$_POST['id']*5;
			$b=5;
			$stmt = $dbCon->prepare("select id,concat_ws(' ',first_name,last_name),email from employees  where company_id=? limit ?,? ");
			...
			while($row = $result->fetch_assoc())
			{
				...
				$org=$org.'<a href="../updateBookingRules/'.$data['cid'].'/'.$enc.'"><div class="column_m container  marb5   fsz14 dark_grey_txt">
										...
										<div class="container padtb15  brdrad1 fsz14 dark_grey_txt padrl10">
											<div class="passport signin_bx dflex fxwrap_w xs-alit_c pos_rel tall">
												...
												<div class="wi_100 xs-wi_100 xs-order_3 xs-padt0 fsz12">
													...
													<span class="edit-text bold padt0 jain2 dblock   brdclr_lgtgrey2 fsz18 xs-padt0 black_txt ">'.$row['name'].'</span>
													  <span class="edit-text   jain2 dblock   brdclr_lgtgrey2 fsz14  black_txt">'.$row['email'].'</span>  </div>
