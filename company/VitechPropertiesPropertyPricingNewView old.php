   ...
   function submitForm()
		{
			...
			if($("#selling_price").val()==0)
			{
				...
				$("#errorMsg1").html('Please enter sell price');
				return false;
			}
			}
			
			 if($('#share_commission_info').val()==1)
			{
			if($("#shared_commision_fee").val()==0)
			{
				
				$("#errorMsg1").html('Please enter shared commision fee');
				return false;
			}
			}
...
<div class="checkout__list">
 ...
 <div class="upload__head" style="margin-bottom:0px;">
                  <h2 class="upload__title h2 white_txt">Property pricing</h2>  
				  
                </div>
				<div class="browse__info info changedText padb40" style="border-bottom: 2px solid #353945;"><?php echo $fetchPropertiesAddress['streetAddress'].','.$fetchPropertiesAddress['postTown'].','.$fetchPropertiesAddress['country'].','.$fetchPropertiesAddress['zipCode']; ?>
</div>
<form class="checkout__item js-tabs-item" action="../../updatePropertyPricing/<?php echo $data['cid']; ?>/<?php echo $data['pid']; ?>" method="POST" id="save_indexing" name="save_indexing" accept-charset="ISO-8859-1" style="display: block;">
 <script>
										function updateRent(link)
										   {
											  body = $('body');
											   if($('#rent_info').val()==1)
											   {
												    $('#bodyData').addClass('dark');
												   $("#rentInfo").addClass('hidden');
												     $(link).attr('checked',''); 
													$('#rent_info').val(0);													 
											   }
											   else
												   {
												   $("#rentInfo").removeClass('hidden');
												     $('#bodyData').addClass('dark');
						  ...
						  <input type="hidden" id="rent_info" name="rent_info" value="<?php echo $fetchPropertiesPricing['rent_info']; ?>"/>
						  
						<div class="footer__theme margin-bottom24 mart40 padb20 uppercase" style="display:block; border-bottom:1px solid #353945; color:white;" >
                DO YOU WANT TO RENT OUT<label class="theme js-theme fright">
                  <input class="theme__input" type="checkbox" onclick='updateRent(this);' <?php if($fetchPropertiesPricing['rent_info']==1) echo 'checked'; ?>><span class="theme__inner"><span class="theme__box"></span></span>
                </label>
              </div>
						  
						  <div class="<?php if($fetchPropertiesPricing['rent_info']==0) echo 'hidden'; ?>" id="rentInfo">
						  
						  	 <script>
									function updateMarkupInfo(link)
										   {
											  body = $('body');
											   if($('#markup_info').val()==1)
											   {
												    $('#bodyData').addClass('dark');
	  ...
	  <input type="hidden" id="markup_info" name="markup_info" value="<?php echo $fetchPropertiesPricing['markup_info']; ?>"/>
						<div class="footer__theme margin-bottom24 padb20 uppercase" style="display:block; border-bottom:1px solid #353945; color:white;" >
                Do you want to allow reseller higher pricing<label class="theme js-theme fright">
                  <input class="theme__input" type="checkbox" onclick='updateMarkupInfo(this);' <?php if($fetchPropertiesPricing['markup_info']==1) echo 'checked'; ?>><span class="theme__inner"><span class="theme__box"></span></span>
                </label>
              </div>
						  
						  
						  <script>
									function updateShareCommisionInfo(link)
										   {
											  body = $('body');
											   if($('#share_commission_info').val()==1)
											   {
												    ...
												    $('#commissionInfo').addClass('hidden');
												     $(link).attr('checked',''); 
													$('#share_commission_info').val(0);	
													$('#shared_commision_fee').val(0);	
											   }
											   else
												   {
	  ...
	  <input type="hidden" id="share_commission_info" name="share_commission_info" value="<?php echo $fetchPropertiesPricing['share_commission_info']; ?>"/>
						<div class="footer__theme margin-bottom24 padb20 uppercase" style="display:block; border-bottom:1px solid #353945; color:white;" >
                Do you want to Share commission?<label class="theme js-theme fright">
                  <input class="theme__input" type="checkbox" onclick='updateShareCommisionInfo(this);' <?php if($fetchPropertiesPricing['share_commission_info']==1) echo 'checked'; ?>><span class="theme__inner"><span class="theme__box"></span></span>
                </label>
              </div>
						  
					 <div class="field margin-bottom24 <?php if($fetchPropertiesPricing['share_commission_info']==0) echo 'hidden'; ?>" id="commissionInfo">
                            <div class="field__label  changedText" style="color:#787f91;">How much share commission.</div>
                            <div class="field__wrap">
							 <select class="" id="shared_commision_fee" name="shared_commision_fee" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								  <?php for($i=0; $i<=30; $i++) { ?>
											<option value="<?php echo $i; ?>"  <?php if($fetchPropertiesPricing['shared_commision_fee']==$i) echo 'selected'; ?>><?php echo $i; ?>%</option>
								  <?php } ?>	 
								  </select>
						   ...
						   <div class="field margin-bottom24 " >
                            <div class="field__label  changedText" style="color:#787f91;">Owner offer commission or the guest?</div>
                            <div class="field__wrap">
							 <select class="" id="commission_from_owner" name="commission_from_owner" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								   
									<option value="1"  <?php if($fetchPropertiesPricing['commission_from_owner']==1) echo 'selected'; ?>>Owner</option>
									<option value="2"  <?php if($fetchPropertiesPricing['commission_from_owner']==2) echo 'selected'; ?>>Guest</option>
                        ...
                        <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">Checkin time</div>
                            <div class="field__wrap">
                              <input class="field__input" type="time" name="arrival_time" id="arrival_time"  style=" border-color: #353945; color:grey;" fdprocessedid="0myzvs" value="<?php echo $fetchPropertiesPricing['arrival_time']; ?>">
                            </div>
                          </div>
						  
						   <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">Checkout time</div>
                            <div class="field__wrap">
                              <input class="field__input" type="time" name="departure_time" id="departure_time" style=" border-color: #353945; color:grey;" fdprocessedid="p0tyog" value="<?php echo $fetchPropertiesPricing['departure_time']; ?>">
                            </div>
                          </div>
						  </div>
						   ...
						   <div class="field margin-bottom24" >
                            <div class="field__label  changedText" style="color:#787f91;">Do you allow pets in the property?</div>
                            <div class="field__wrap">
                               <select class="" id="pets_info_detail" name="pets_info_detail" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"  fdprocessedid="zc7mun">
								  
											<option value="1"  <?php if($fetchPropertiesPricing['pets_allowed']==1) echo 'selected'; ?>>Yes</option>
											<option value="0" <?php if($fetchPropertiesPricing['pets_allowed']==0) echo 'selected'; ?>>No</option>
								  </select>
                            </div>
                          </div> 	  
						 <div class="field margin-bottom24" >
                            <div class="field__label  changedText" style="color:#787f91;">Do you allow disabled person in the property?</div>
                            <div class="field__wrap">
                               <select class="" id="disable_info_detail" name="disable_info_detail" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								  
											<option value="1"  <?php if($fetchPropertiesPricing['disabled_allowed']==1) echo 'selected'; ?>>Yes</option>
											<option value="0" <?php if($fetchPropertiesPricing['disabled_allowed']==0) echo 'selected'; ?>>No</option>
								  </select>
                            </div>
                          </div> 	
						  
						  <div class="field margin-bottom24" >
                            <div class="field__label  changedText" style="color:#787f91;">Do you allow children in the property?</div>
                            <div class="field__wrap">
                               <select class="" id="children_info_detail" name="children_info_detail" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								  
											<option value="1"  <?php if($fetchPropertiesPricing['children_allowed']==1) echo 'selected'; ?>>Yes</option>
											<option value="0" <?php if($fetchPropertiesPricing['children_allowed']==0) echo 'selected'; ?>>No</option>
								  </select>
                            </div>
                          </div> 	
						  
						   <div class="field margin-bottom24" >
                            <div class="field__label  changedText" style="color:#787f91;">Do you allow infants in the property?</div>
                            <div class="field__wrap">
                               <select class="" id="infants_info_detail" name="infants_info_detail" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								  
											<option value="1"  <?php if($fetchPropertiesPricing['infants_allowed']==1) echo 'selected'; ?>>Yes</option>
											<option value="0" <?php if($fetchPropertiesPricing['infants_allowed']==0) echo 'selected'; ?>>No</option>
								  </select>
                            </div>
                          </div> 
						  
						   <div class="field margin-bottom24" >
                            <div class="field__label  changedText" style="color:#787f91;">If Smoking is allowed in the property?</div>
                            <div class="field__wrap">
                               <select class="" id="smoking_info_detail" name="smoking_info_detail" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								  
											<option value="1"  <?php if($fetchPropertiesPricing['smoking_allowed']==1) echo 'selected'; ?>>Yes</option>
											<option value="0" <?php if($fetchPropertiesPricing['smoking_allowed']==0) echo 'selected'; ?>>No</option>
								  </select>
                            </div>
                          </div> 
						  
						   <div class="field margin-bottom24" >
                            <div class="field__label  changedText" style="color:#787f91;">If parties/events are allowed in the property?</div>
                            <div class="field__wrap">
                               <select class="" id="parties_info_detail" name="parties_info_detail" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun">
								  
											<option value="1"  <?php if($fetchPropertiesPricing['parties_allowed']==1) echo 'selected'; ?>>Yes</option>
											<option value="0" <?php if($fetchPropertiesPricing['parties_allowed']==0) echo 'selected'; ?>>No</option>
								  </select>
                            </div>
                          </div> 
                        ...
                        <div class="field" style="flex: 0 0 calc(33% - 16px);    width: calc(33% - 16px);    margin: 0 8px;">
                            <div class="field__label  changedText" style="color:#787f91;">Owner pricing (per night)</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="price_per_night" id="price_per_night" style=" border-color: #353945; color:grey;" fdprocessedid="fu8qmc" min=0 value="<?php echo $fetchPropertiesPricing['owner_price_per_night']; ?>">
                            </div>
                          </div>
						  
						   <div class="field" style="flex: 0 0 calc(33% - 16px);    width: calc(33% - 16px);    margin: 0 8px;">
                            <div class="field__label  changedText" style="color:#787f91;">Deposit fee</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="deposit_fee" id="deposit_fee" style=" border-color: #353945; color:grey;" fdprocessedid="tqjibs" min=0 value="<?php echo $fetchPropertiesPricing['owner_deposit_fee']; ?>">
                            </div>
                          </div>
						  
						   <div class="field" style="flex: 0 0 calc(33% - 16px);    width: calc(33% - 16px);    margin: 0 8px;">
                            <div class="field__label  changedText" style="color:#787f91;">Cleaning fee</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="cleaning_fee" id="cleaning_fee" style=" border-color: #353945; color:grey;" fdprocessedid="tqjibs" min=0 value="<?php echo $fetchPropertiesPricing['owner_cleaning_fee']; ?>">
                            </div>
                          </div>
						   ...
						   <div class="checkout__row margin-bottom24">
                        <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">Seller pricing (per night)</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="seller_price_per_night" id="seller_price_per_night" style=" border-color: #353945; color:grey;" fdprocessedid="fu8qmc" min=0 value="<?php echo $fetchPropertiesPricing['seller_price_per_night']; ?>">
                            </div>
                          </div>
						   ...
						   <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">Deposit fee</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="seller_deposit_fee" id="seller_deposit_fee" style=" border-color: #353945; color:grey;" fdprocessedid="tqjibs" min=0 value="<?php echo $fetchPropertiesPricing['seller_deposit_fee']; ?>">
                            </div>
                          </div>
						   ...
						   <div class="checkout__row margin-bottom24">
                        <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">Seller cleaning fee</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="seller_cleaning_fee" id="seller_cleaning_fee" style=" border-color: #353945; color:grey;" fdprocessedid="fu8qmc" min=0 value="<?php echo $fetchPropertiesPricing['seller_cleaning_fee']; ?>">
                            </div>
                          </div>
						  
						   <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">late arrival fee</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="late_arrival_fee" id="late_arrival_fee" style=" border-color: #353945; color:grey;" fdprocessedid="tqjibs" min=0 value="<?php echo $fetchPropertiesPricing['late_arrival_fee']; ?>">
                            </div>
                          </div>
						   ...
						   <div class="checkout__row margin-bottom24">
                        <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">Security fee</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="seller_security_fee" id="seller_security_fee" style=" border-color: #353945; color:grey;" fdprocessedid="fu8qmc" min=0 value="<?php echo $fetchPropertiesPricing['seller_security_fee']; ?>">
                            </div>
                          </div>
						  
						   <div class="field" >
                            <div class="field__label  changedText" style="color:#787f91;">Tourist tax(%)</div>
                            <div class="field__wrap">
                               <select class="" id="tourist_tax" name="tourist_tax" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;" onchange="updateSell(this.value);" fdprocessedid="zc7mun">
								 <?php for($i=0;$i<=20;$i++) { ?>	
													 <option value="<?php echo $i; ?>" <?php if($fetchPropertiesPricing['tourist_tax']==$i) echo 'selected'; ?>><?php echo $i; ?></option><?php } ?> 
								  </select>
                            </div>
                          </div> 
						   ...
						   <script>
						    ...
						    <div class="field margin-bottom24" >
                            <div class="field__label  changedText" style="color:#787f91;">Do you parking available in the property?</div>
                            <div class="field__wrap">
                               <select class="" id="parking_info_detail" name="parking_info_detail" style="height: 48px;
								padding: 0 14px; border-color: #353945;
								color: grey; border-radius: 12px;
								  background: transparent; width:100%;"   fdprocessedid="zc7mun" onchange="updateParkingType(this.value);">
								  
											<option value="1"  <?php if($fetchPropertiesPricing['parking_info_detail']==1) echo 'selected'; ?>>Yes</option>
											<option value="0" <?php if($fetchPropertiesPricing['parking_info_detail']==0) echo 'selected'; ?>>No</option>
								  </select>
                            </div>
                          </div> 
						  <div class="field margin-bottom24 <?php if($fetchPropertiesPricing['parking_info_detail']==0) echo 'hidden'; ?>" id="parkingInfo">
                            <div class="field__label  changedText" style="color:#787f91;">How much do you charge for parking.</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" id="parking_fee" name="parking_fee" min=0 style=" border-color: #353945; color:grey;" fdprocessedid="vmdbg5" min=0 value="<?php echo $fetchPropertiesPricing['parking_fee']; ?>">
                            </div>
                          </div>
						   ...
						   <script>
									function updateSell(link)
										   {
											  body = $('body');
											   if($('#sell_info').val()==1)
											   {
												    $('#bodyData').addClass('dark');
												   $("#sellInfo").addClass('hidden');
												     $(link).attr('checked',''); 
													$('#sell_info').val(0);													 
											   }
											   else
												   {
												   $("#sellInfo").removeClass('hidden');
												     $('#bodyData').addClass('dark');
												     $(link).attr('checked','checked');  
													 $('#sell_info').val(1);	
												   }
	  ...
	  <input type="hidden" id="sell_info" name="sell_info" value="<?php echo $fetchPropertiesPricing['sell_info']; ?>"/>
						<div class="footer__theme margin-bottom24 padb20 uppercase" style="display:block; border-bottom:1px solid #353945; color:white;" >
                Do you want to sell<label class="theme js-theme fright">
                  <input class="theme__input" type="checkbox" onclick='updateSell(this);' <?php if($fetchPropertiesPricing['sell_info']==1) echo 'checked'; ?>><span class="theme__inner"><span class="theme__box"></span></span>
                </label>
              </div>
						<div class="field margin-bottom24 <?php if($fetchPropertiesPricing['sell_info']==0) echo 'hidden'; ?>" id="sellInfo">
                            <div class="field__label  changedText" style="color:#787f91;">Selling price</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" id="selling_price" name="selling_price" min=0 style=" border-color: #353945; color:grey;" fdprocessedid="vmdbg5" value="<?php echo $fetchPropertiesPricing['selling_price']; ?>">
                            </div>
                          </div>
						
						 <script>
									function updateManage(link)
										   {
											  body = $('body');
											   if($('#manage_info').val()==1)
											   {
												    $('#bodyData').addClass('dark');
	  ...
	  <input type="hidden" id="manage_info" name="manage_info" value="<?php echo $fetchPropertiesPricing['manage_info']; ?>"/>
						<div class="footer__theme margin-bottom24 padb20 uppercase" style="display:block; border-bottom:1px solid #353945; color:white;" >
                Do you want to manage<label class="theme js-theme fright">
                  <input class="theme__input" type="checkbox" onclick='updateManage(this);' <?php if($fetchPropertiesPricing['manage_info']==1) echo 'checked'; ?>><span class="theme__inner"><span class="theme__box"></span></span>
                </label>
              </div>
...
<div class="upload__preview js-upload-preview">
                <div class="upload__wrap" style="background:transparent;">
                  <button class="upload__close js-upload-close">
                    <svg class="icon icon-close">
                      <use xlink:href="#icon-close"></use>
                    </svg>
                  </button>
                  <div class="upload__subtitle">Preview</div>
				  <a class="card" href="#" style="background:transparent;">
                    ...
                    <div class="card__body" style="background:transparent;">
                      <div class="card__line">
                        <div class="card__title"><?php echo $fetchPropertyTaskInfo['shortSaleDescription']; ?></div>
                        <div class="card__price">
                          
                          <div class="card__actual"><?php echo $fetchPropertyTaskInfo['livingSpace']; ?> sqm</div>
                        </div>
                      </div>
                      <div class="card__options">
                        <div class="card__option">
                          ...
                          </svg><?php echo $fetchPropertiesInterior['numberOfBedrooms']; ?> bedrooms
                        </div>
                        <div class="card__option">
                          <svg class="icon icon-burger">
                            <use xlink:href="#icon-burger"></use>
                          </svg><?php echo $fetchPropertiesInterior['numberOfBathrooms']; ?> bathroom
                        </div>
                      </div>
                      <div class="card__foot">
                        <div class="card__flex">
                          <div class="card__cost">$<?php echo $fetchPropertyTaskInfo['owner_sell_price']; ?></div>
