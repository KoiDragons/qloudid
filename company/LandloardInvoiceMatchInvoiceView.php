...
<script>
		...
		function checkEmail(id) {
			...
			function submitform()
			{
				...
				$("#errorMsg1").html('');
				 if($("#invoice_detail").val()==0)
				{
				$("#errorMsg1").html('Please select matching invoice');	
				return false;
				}
...
<div class="travel__subtitle changedText " style="
            ...
            <div class="travel__content changedText xs-fsz27" style=" line-height: 1.4;
                  color: #787f91;
				   line-height: 1.4;
                  font-size: 27px;
                  ">Select invoice for the selected payment
</div>
...
<form action="../../updateInvoicePayment/<?php echo $data['cid']; ?>/<?php echo $data['payment_id']; ?>" method="POST" id="save_indexing1" name="save_indexing1" accept-charset="ISO-8859-1">
...
<div class="field">
<div class="field__label changedText" style="color:#787f91;">Invoice</div>
<div class="field__wrap">
<select  class="   "  name="invoice_detail" id="invoice_detail" style="height: 48px;
    padding: 0 14px; border-color: #353945;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;" fdprocessedid="8mc52"  >
	  <option value="0">Select</option>
				 <?php $i=1; 
								foreach($landloardUnpaidInvoices as $category => $value) 
										{
											 
										 ?> 
               
                         	<option value="<?php echo $value['id']; ?>">No <?php echo $value['id']; ?> $<?php echo $value['payment_amount']; ?></option>
										<?php } ?>
                          </select>
</div>
</div>
...
<a class="button checkout__button changedText  " href="javascript:void(0);" onclick="submitform();">Confirm payment</a>
