...
class UserSupplierCompanyModel extends AppModel
{
	...
	function requestDetailApproved($data)
    {
        $dbCon = AppModel::createConnection();
      $company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
        $stmt = $dbCon->prepare("select user_suppliers.user_login_id,user_suppliers.id,company_name,last_name,first_name,email,user_suppliers.created_on from user_suppliers left join user_logins on user_logins.id=user_suppliers.user_login_id left join companies on companies.id=user_suppliers.company_id where company_id=? and is_approved=1 limit 0,20");
