...
<script>
			function whoChange(id)
			{
				//alert(id);
				if(id==0)
				{
					$("#whoDrop").attr('style','display:none');
				}
				else
				{
				$("#whoDrop").attr('style','display:block');	
				}
			}
			...
			function sendInformation()
			{
				...
				if($("#chld").val()==0)
				{
					...
					$("#error_msg_form").html('Please select child');
					return false;
				}
				if($("#who").val()==0)
				{
					...
					document.getElementById("person_informed").style.display='block';
					$(".person_informed").addClass('active');
					$("#error_msg_form").html('Please select who droped off child');
					return false;
				}
...
<body class="white_bg">
		...
		<div class="column_m header xs-header xsip-header xsi-header bs_bb lgtgrey2_bg">
			...
			<div class="fright xs-dnone sm-dnone padt10 padb10">
					<ul class="mar0 pad0 sf-menu fsz14">
						...
						<li class="dblock hidden-xs hidden-sm fleft pos_rel marl20"> <a href="#" id="usermenu_singin" class="diblock padl20 padr10 brdrad3 transbg  lgn_hight_29 fsz14 black_txt" data-en="Stäng" data-sw="Stäng">Stäng</a> </li>
						
					</ul>
				</div>
			<div class="visible-xs visible-sm fright marr0 padr0 padt10  brdwi_3"> <a href="#" class="diblock padl20 padr10 brdrad3 transbg  lgn_hight_29 fsz14 black_txt">Stäng</a> </div>
				<div class="clear"></div>
			</div>
		</div>
	<div class="clear"></div>
	...
	<div class="container-fluid full-height xsi-mart40 xs-mart40" onclick="checkFlag();">
		<div class="row row-height">
			
			<div class="column_m container zi5  mart40 xs-mart40 "  >
				<div class="wrap maxwi_100 padrl15 md-padrl0 lg-padrl0">
					
					
					<!-- Center content -->
					<div class="wi_550p maxwi_100 col-xs-12 col-sm-12 pos_rel zi5 marrla pad10 mart40 xs-pad0">
					
						<div class="padb20 xxs-padb0 ">	
					
						
							<form action="../signInReceivedChild/<?php echo $data['cid']; ?>" method="POST" name="save_indexing" id="save_indexing">
							<!-- Leave for security protection, read docs for details -->
							<div class="hei_160p">
								<div class="step">
									<h3 class="marb0  xxs-talc talc fsz35 xs-fsz25 bold padb5 black_txt ">Sign in child</h3>
									<div class="marb20   xxs-talc talc"> <a href="#" class="padb10 brdb_new black_txt fsz20 xs-fsz20 xxs-talc edit-text jain_drop_company" >Select child and provide information who dropped him.</a></div>
								<div class="form-group">
										<div class="styled-select clearfix">
											<select class="txtind10 default_view wi_100 mart0 lgtgrey_bg nobrdr nobrdt nobrdl brdb  fsz18  minhei_55p  dark_grey_txt tall padl0" name="chld" id="chld" onchange="whoChange(this.value);">
											<option value="0" class="lgtgrey2_bg">Select</option>
												<?php  foreach($selectChild as $category => $value) 
															{
																$category = htmlspecialchars($category); 
															?>
															
															<option value="<?php echo $value['id']; ?>" class="lgtgrey2_bg"><?php echo $value['name']; ?></option>
														<?php 	}	?>                            
											</select>
										</div>
									</div>
									
									<div class="form-group padt20 " style="display:none;" id="whoDrop">
									<label class="fsz18 fleft">Who dropped this kid off?</label>
										<div class="styled-select clearfix">
											<select class="txtind10 default_view wi_100 mart0 lgtgrey_bg nobrdr nobrdt nobrdl brdb  fsz18  minhei_55p  dark_grey_txt tall padl0" name="who" id="who">
												
															
															<option value="Dont Know Who" class="lgtgrey2_bg">Dont Know Who</option>
														   <option value="Relatives" class="lgtgrey2_bg">Relatives</option>                        
											</select>
										</div>
									</div>
							...
							<div class="clear"></div>
							<!-- /middle-wizard -->
							<div class="padtb20 xxs-talc talc mart35">
								
								<button type="button" name="forward" class="forward" onclick="sendInformation();">Registrera</button>
