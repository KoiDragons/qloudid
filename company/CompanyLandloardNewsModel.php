...
class CompanyLandloardNewsModel extends AppModel
{
    ...
    {
        $dbCon = AppModel::createConnection();
		$landlord_id= $this -> encrypt_decrypt('decrypt',$data['tid']);
        $stmt = $dbCon->prepare("select company_landloard.id,company_name,visiting_address,company_landloard.created_on,is_headquarter,requesting_company from company_landloard left join property_location on property_location.id=company_landloard.property_id left join companies on companies.id=company_landloard.receiving_company where company_landloard.id=?");
