...
class HotelDryCleaningSearchModel extends AppModel
{
		function appId()
		{
	...
	function sendRequest($data)
	{
		 ...
		 $drycleaning_id= $this -> encrypt_decrypt('decrypt',$data['id']);  
				$stmt = $dbCon->prepare("select company_id from drycleaning_center_information  where id=?");
				...
				$stmt = $dbCon->prepare("INSERT INTO hotel_drycleaning_services (drycleaning_id, hotel_company_id, created_on,hotel_property_id,is_roomservice) VALUES (?, ?, now(),?, ?)");
				$stmt->bind_param("iiii", $drycleaning_id,$company_id,$location_id,$st);
				$stmt->execute();
				$url="https://www.safeqloud.com/company/index.php/DryCleaning/roomServiceRequests/".$cid."/".$data['id'];
  ...
  sendEmail($subject, $to, $emailContent);
    ...
    function resturantListSearch($data)
    {
		...
		$p="%".htmlentities($_POST['message'],ENT_NOQUOTES, 'ISO-8859-1')."%";
					...
					$stmt = $dbCon->prepare("select drycleaning_center_information.id,center_name,visiting_address  from drycleaning_center_information left join property_location on property_location.id=drycleaning_center_information.property_id   where center_name like ?  and property_location.city_v=(select property_location.city_v from property_location where property_location.id=?) and drycleaning_center_information.id not in (select drycleaning_id from hotel_drycleaning_services where hotel_property_id=?)");
	 ...
	 function resturantListCount($data)
    {
		...
		$p="%".htmlentities($_POST['message'],ENT_NOQUOTES, 'ISO-8859-1')."%";
					...
					$stmt = $dbCon->prepare("select count(drycleaning_center_information.id) as num  from drycleaning_center_information left join property_location on property_location.id=drycleaning_center_information.property_id   where center_name like ?  and property_location.city_v=(select property_location.city_v from property_location where property_location.id=?) and drycleaning_center_information.id not in (select drycleaning_id from hotel_drycleaning_services where hotel_property_id=?)");
				 $stmt->bind_param("sii", $p,$location_id,$location_id);
	 ...
	 function requestInfo($data)
    {
		...
		$location_id= $this -> encrypt_decrypt('decrypt',$data['lid']);
		$drycleaning_id= $this -> encrypt_decrypt('decrypt',$data['id']);
		$stmt = $dbCon->prepare("select is_roomservice from hotel_drycleaning_services where hotel_property_id=? and drycleaning_id=?");
		$stmt->bind_param("ii", $location_id,$drycleaning_id);
		$stmt->execute();
		$result = $stmt->get_result();
		$row=mysqli_fetch_array($result);
		$stmt->close();
        $dbCon->close();
		return $row['is_roomservice'];
	}
		 ...
		 function roomServiecRequestSent($data)
    {
		...
		$location_id= $this -> encrypt_decrypt('decrypt',$data['lid']);
		ini_set('memory_limit', '-1');
		 			$stmt = $dbCon->prepare("select drycleaning_center_information.id,center_name,visiting_address  from drycleaning_center_information left join property_location on property_location.id=drycleaning_center_information.property_id   where  drycleaning_center_information.id in (select drycleaning_id from hotel_drycleaning_services where hotel_property_id=?)");
