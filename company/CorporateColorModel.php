<?php
	...
	class CorporateColorModel extends AppModel
	{
		...
		function addCorporateColor($data)
		{
			$dbCon = AppModel::createConnection();
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			
			$stmt = $dbCon->prepare("select id from corporate_color where company_id=?");
			...
			if(empty($row))
			{
				$stmt = $dbCon->prepare("INSERT INTO corporate_color (company_id, created_on,bg_color,f_color ) VALUES (?, now(),? ,?)");
				
				
				$stmt->bind_param("iss", $company_id,$_POST['bg_color'],$_POST['f_color']);
				$stmt->execute();
			}
			else
			{
			$stmt = $dbCon->prepare("update corporate_color set bg_color=?,f_color=? where id=?");
			
			/* bind parameters for markers */
			$stmt->bind_param("ssi",$_POST['bg_color'],$_POST['f_color'], $row['id']);
