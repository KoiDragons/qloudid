  ...
  function getValue()
		 {
			...
			var newEmployee='<div class="upload__item" id="'+counter_value+'"><div class="field"> <div class="field__label  changedText" style="color:#787f91;">Employee email</div><div class="field__wrap"><input class="field__input" type="text" name="employee_email'+counter_value+'" id="employee_email'+counter_value+'" placeholder="e. g. xyz@gmail.com"  style=" color:grey;"  > </div></div>  </div>'; 
  ...
  function submitform()
		{
			...
			$("#errorMsg1").html('');
			if($("#more_employee_available").val()==1)
			{
			var counter_value=parseInt($('.js-counter-babies').val()); 
			 for(i=1;i<=counter_value;i++)
			 {
				 if($('#employee_email'+i).val()=='' || $('#employee_email'+i).val()==null)
				 {
					$("#errorMsg1").html('Please enter employee email'); 
					return false;
				 }
			 }	
...
<button class="header__burger js-header-burger"></button>
 ...
 <!-- outer content-->
	  ...
	  <div class="control center" style="margin-bottom:0px;  max-width: 1132px;">
	   <?php if($_POST['proposal_type']==3) { ?>
	  <a class="button-stroke button-small control__button" href="../../LandloardBroker/listBrokerProposalSociety/<?php echo $data['cid']; ?>">
	  <?php } else { ?>
	   <a class="button-stroke button-small control__button" href="../listAddedProperties/<?php echo $data['cid']; ?>">
	   <?php } ?>
...
<div class="travel__center center   padb136 xxs-padrl25 " style="border-bottom:2px solid #23262F;  max-width: 1132px;
     margin-bottom:40px; padding-bottom:40px;  padding-left:0px; padding-right:0px;">
   
   <div class="travel__row " style="margin-bottom: 15px;">
      <div class="travel__col xs-marb0 <?php if($companyDetail['more_employee_available']==0) echo 'marb0'; else 'marb175'; ?>" style="padding-right: 0px;  max-width:595px;">
         <div class="travel__list" style="
            ...
            <div class="travel__content changedText xs-fsz27" style=" line-height: 1.4;
                  color: #787f91;
				   line-height: 1.4;
                  font-size: 27px;
                  ">Select the team member you want to show on proposal</div>
...
<div class="checkout__list">
  ...
  <div class="setting__item  " style="display:block;">
          <script>       function updateHeading(id)
			{
				if($("#moreEmployees").hasClass('hidden'))
				{
					$("#team_available").val(1);
					$("#moreEmployees").removeClass('hidden');
				}
				else
				{
					$("#team_available").val(0);
					$("#moreEmployees").addClass('hidden');
				}
  ...
  <div class="setting__box marb40">
                    <div class="setting__notification" style="border:none; margin-bottom:0px;">
                      <div class="setting__main">
                        <div class="setting__subtitle changedText">Do you have want to show team</div>
                        <label class="switch">
   ...
   <div class="section travels">
    ...
    border-radius: 0px;
box-shadow:none;">
               <div class="travels__list hidden" id="moreEmployees" >
			   <?php $i=1; 
								foreach($listTeam as $category => $value) 
										{
											  
										 ?> 
			   <div class="travels__card  bg10161d " id="p<?php echo $value['id']; ?>">
                                  <div class="travels__preview"><img srcset="<?php echo $value['photo_info']; ?>" src="<?php echo $value['photo_info']; ?>" src="<?php echo $value['photo_info']; ?>" src="<?php echo $value['photo_info']; ?>" alt="Card" style="border-radius: 50%;
    padding: 20px 40px;">
                                  ...
                                  <div class="travels__body">
                                    <div class="travels__line" style="border:none; margin:0px; padding:0px;">
                                      <div class="travels__details">
                                        <div class="travels__subtitle" style="color: #777E90;"><?php echo $value['first_name'].' '.$value['last_name']; ?></div>
                                        <div class="travels__location" style="color: #777E90;">Employee</div>
                                      </div>
   ...
   " onclick="selectWishlist(<?php echo $value['id']; ?>);">Select</a>
                                     ...
                                     <div class="travels__line hidden">
                                      <div class="travels__date"><?php if(empty($value['work_email'])) echo '-'; else echo $value['work_email']; ?> & <?php if(empty($value['wmobile'])) echo '-'; else echo $value['mobile']; ?></div>
			  ...
			  <script>
										...
										{
											$('error_msg_form').html('');
											if($('#team_available').val()==1)
											{
											if($('#team_member').val()=='')
											{
											$('error_msg_form').html('Please select atleast one team member');	
											return false;
											}	
											}	
                ...
                <form class="checkout__item js-tabs-item talc" action="../<?php if($_POST['proposal_type']==1) echo 'createPropertyProposal'; else if($_POST['proposal_type']==3 || $_POST['proposal_type']==4) echo 'createPropertyProposal'; else echo 'createProposal'; ?>/<?php echo $data['cid']; ?>" method="POST" id="save_indexing_proposal" name="save_indexing_proposal" accept-charset="ISO-8859-1" style="display: block;">		
				<input type="hidden" id="team_member"	name="team_member" value="" />
					 				<input type="hidden" id="proposal_data"	name="proposal_data" value="<?php echo $_POST['proposal_data']; ?>" />
									<input type="hidden" id="proposal_type"	name="proposal_type" value="<?php echo $_POST['proposal_type']; ?>" />
									<input class="field__input" type="hidden" name="team_available" id="team_available" placeholder="e. g. xyz@gmail.com"  style=" color:grey;" value="0" >
									<div id="error_msg_form" class="fsz20 red_txt padtb20 tall"></div>
									<a class="button checkout__button changedText  " href="javascript:void(0);" style="padding: 16px; 24px;
