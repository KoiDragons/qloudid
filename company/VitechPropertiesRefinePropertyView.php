...
<body class="dark" style="background-color:#131922">
  ...
  function updateInvoice(id,link)
   ...
   var optionsDetail='<option value="0" class="changedText">Select</option>';
   ...
   function selectService(id)
					...
					$.ajax({
						type:"POST",
						url:"../../selectService/<?php echo $data['cid']; ?>",
						data:send_data,
						dataType:"text",
						contentType: "application/x-www-form-urlencoded;charset=utf-8",
						success: function(data1){
							
							 $('#subcategory_id').html(data1);
							  $('#property_status').html(optionsDetail);
						}
					});
					}

 function updatePropertyStatus(id)
					{
					var send_data={};
					send_data.id=id;
					...
					$.ajax({
						type:"POST",
						url:"../../updatePropertyStatus/<?php echo $data['cid']; ?>",
						data:send_data,
						dataType:"text",
						contentType: "application/x-www-form-urlencoded;charset=utf-8",
						success: function(data1){
							
							 $('#property_status').html(data1);
							updateStatus(); 
						}
					});
					}
   ...
   function informEmployee()
			{
				...
				{
				$("#error_msg_form").html('Please select category of the property');
					return false;	
				}
				...
				{
				$("#error_msg_form").html('Please select subcategory of the property');
					return false;	
				}
				 
				if($("#property_status").val()==0)
				{
				$("#error_msg_form").html('Please select status of the property');
					return false;	
				}
				...
				if($("#contract_type").val()==0)
				{
					...
					$("#error_msg_form").html('Please select contract type');
					return false;
				}
				...
				if($("#start_date").val()=="" || $("#start_date").val()==null)
				{
					...
					$("#error_msg_form").html('Please enter contract start date');
					return false;
				}
				...
				if($("#end_date").val()=="" || $("#end_date").val()==null)
				{
					...
					$("#error_msg_form").html('Please enter contract end date');
					return false;
				}
				
				  
					var send_data={};
					send_data.move_out_date=$("#start_date").val();
					send_data.move_in_date=$("#end_date").val(); 
					$.ajax({
						type:"POST",
						url:"../../checkDates",
						data:send_data,
							 ...
							 if(data1==0)
								{
									...
									$("#error_msg_form").html('contract end date cant be before start date. Please select different dates.');
									return false;
								}
								else
								{
									var send_data={};
									send_data.category_id=$("#category_id").val();
									send_data.subcategory_id=$("#subcategory_id").val();
									send_data.property_status=$("#property_status").val();
									send_data.apartment_number=$("#apartment_number").val();
									send_data.addrs=$("#addrs").val();
									send_data.pnumber=$("#pnumber").val();					
									$.ajax({
										type:"POST",
										url:"../../checkApartmentContractAvailability",
										data:send_data,
										dataType:"text",
										contentType: "application/x-www-form-urlencoded;charset=utf-8",
										success: function(data1){
											 ...
											 if(data1>0)
											 {
												$("#error_msg_form").html('You already have sent this request for this apartment. Please select another service.');
												return false;
											 }
											 else
											 {
...
<a class="header__logo xs-fsz25 changedText" href="../../listImportedProperties/<?php echo $data['cid']; ?>" style="
...
<button class="header__head js-header-head fsz14 hidden-xs changedText bold" style="color:#777E90"><?php echo $companyDetail['company_name']; ?></button>
            ...
            <div class="header__body js-header-body">
              <div class="header__group">
                ...
                <div class="header__menu">
					...
					<a class="header__link" href="https://www.safeqloud.com/company/index.php/CompanySuppliers/companyProfileAction/<?php echo $data['cid']; ?>">
...
<div class="travel__subtitle changedText " style="
            ...
            <div class="travel__content changedText xs-fsz27" style=" line-height: 1.4;
                  color: #787f91;
				   line-height: 1.4;
                  font-size: 27px;
                  ">Fill in the form and add your property to our marketplace
</div>
...
<form class="checkout__item js-tabs-item" action="../../refineVitechAdddress/<?php echo $data['cid']; ?>/<?php echo $data['ip_id']; ?>" method="POST" id="save_indexing" name="save_indexing" accept-charset="ISO-8859-1" style="display: block;">
 <input type="hidden" name="came_from" id="came_from" value="8"/> 
 
 
 <div class="upload__item hidden">
					 ...
					 <div class="checkout__row" style="margin-top:20px; margin-bottom:24px;">
                          <div class="field"  >
                            <div class="field__label  changedText" style="color:#787f91;">Category</div>
                            <div class="field__wrap">
                               <select class=" "   id="category_id" name="category_id" style="height: 48px;
    padding: 0 14px;  border-color: #353945;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;"  onchange='selectService(this.value);' >
								  <?php echo $serviceCategoryList; ?>
								  </select>
                            </div>
                          </div>
                          <div class="field"  >
                            <div class="field__label  changedText" style="color:#787f91;">Service</div>
                            <div class="field__wrap">
                              <select class=" "   id="subcategory_id" name="subcategory_id" style="height: 48px;
    padding: 0 14px;  border-color: #353945;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;"   onchange="updatePropertyStatus(this.value);">
								 <option value="0" class="changedText">Select</option>
								  </select>
                            </div>
                          </div>
					 ...
					 <div class="checkout__line  " style="margin-bottom:24px;" >
					   <div class="checkout__subtitle changedText">Status</div>
                       
                      </div>
					  
						    <div class="field" style="margin-bottom:24px;">
                            <div class="field__label  changedText" style="color:#787f91;">Property status</div>
                            <div class="field__wrap">
                             <select class=" " id="property_status" name="property_status" style="height: 48px;
    padding: 0 14px;  border : 2px solid #f67f00;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;" onchange="updateStatus(this.value);" >
							 <option value="0" class="changedText">Select</option>
						  ...
						  {
							  var id=$('#property_status').val();
							  if(id>0)
							  {
								 $('#propertyDetailStatusInfo').removeClass('hidden'); 
								 $('.travel__list').addClass('marb1750');
								 
								 
							  }
							  else 
								 {
								 $('#propertyDetailStatusInfo').addClass('hidden'); 
								 $('.travel__list').removeClass('marb1750');
								 
							  }
							  if(id==1)
							  {
								  
								$('#status_info').removeClass('hidden');
							  }
							  else
								  {
								$('#status_info').addClass('hidden');
							  }
						  }
						  
						  </script>
						 
 <div class="hidden" id="propertyDetailStatusInfo">
						<div class="upload__item">
                          ...
                          <div class="upload__col upload__col_w70">
                            <div class="upload__label changedText">price</div>
                            <div class="upload__line">
                              <div class="upload__cell">
                                <div class="field field_empty">
                                  <div class="field__wrap">
                                    <input class="field__input" type="number" name="sell_price" id="sell_price" value="<?php echo $displaypropertiesDetail['sale_price']; ?>" placeholder="e. g. &quot;180&quot;" required>
                                  </div>
                                </div>
                              </div>
                              <div class="upload__cell">
    ...
    color: grey; border-radius: 12px;
      background: white; width:100%;">
						  ...
						  <div class="upload__col upload__col_w30 " id="status_info">
                            <div class="upload__label changedText">Selling date</div>
                            <div class="upload__line">
                              <div class="upload__cell">
                                <div class="field field_empty">
                                  <div class="field__wrap">
                                    <input class="field__input" type="date" name="sold_on" id="sold_on" style=" color:grey;"  >
                                  </div>
                                </div>
                              </div>
					    ...
					    <div class="checkout__line  " style="margin-bottom:24px;" >
					   <div class="checkout__subtitle changedText">Property type</div>
                      ...
                      <div class="upload__fieldset ">
					   <div class="field hidden">
                            ...
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="sell_phrase" id="sell_phrase" placeholder="e. g. &quot;Spectacular views of Queenstown&quot;" style=" color:grey;"  >
                            </div>
                          </div>
					    <div class="field">
    ...
    color: grey; border-radius: 12px;
      background: transparent; width:100%;">
								   <option value="House" <?php if($displaypropertiesDetail['property_name']=='House') echo 'selected'; ?> class="changedText">House</option>
											<option value="Apartment" <?php if($displaypropertiesDetail['property_name']=='Apartment') echo 'selected'; ?> class="changedText">Apartment</option>
											<option value="Lodge" <?php if($displaypropertiesDetail['property_name']=='Lodge') echo 'selected'; ?> class="changedText">Lodge</option>
											<option value="Cottage" <?php if($displaypropertiesDetail['property_name']=='Cottage') echo 'selected'; ?> class="changedText">Cottage</option>
											<option value="Cabin" <?php if($displaypropertiesDetail['property_name']=='Cabin') echo 'selected'; ?> class="changedText">Cabin</option>
											<option value="Chalet" <?php if($displaypropertiesDetail['property_name']=='Chalet') echo 'selected'; ?> class="changedText">Chalet</option>
											<option value="Wigwam" <?php if($displaypropertiesDetail['property_name']=='Wigwam') echo 'selected'; ?> class="changedText">Wigwam</option>
											<option value="Barn" <?php if($displaypropertiesDetail['property_name']=='Barn') echo 'selected'; ?> class="changedText">Barn</option>
								  </select>
                            </div>
                          </div>
                        ...
                        <div class="field"  >
                            <div class="field__label  changedText" style="color:#787f91;">Apartment number</div>
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="apartment_number" id="apartment_number" value="<?php echo $displaypropertiesDetail['property_id']; ?>"  placeholder="e. g. 1234" style=" color:grey;"  >
                            </div>
                          </div>
						  
						   <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">#Floors</div>
                            <div class="field__wrap">
						   ...
						   <div class="checkout__row">
                          <div class="field hidden" style="flex: 0 0 calc(33% - 16px);    width: calc(33% - 16px);    margin: 0 8px 24px 8px;">
    ...
    color: grey; border-radius: 12px;
      background: transparent; width:100%;">
								 <?php for($i=1; $i<=10; $i++) { ?>
											<option value="<?php echo $i; ?>" <?php if($displaypropertiesDetail['bedrooms']==$i) echo 'selected'; ?>><?php echo $i; ?></option>
								   <?php } ?>
								  </select>
                            </div>
                          </div>
    ...
    color: grey; border-radius: 12px;
      background: transparent; width:100%;">
								<?php for($i=1; $i<=10; $i++) { ?>
											<option value="<?php echo $i; ?>" <?php if($displaypropertiesDetail['bathrooms']==$i) echo 'selected'; ?>><?php echo $i; ?></option>
								   <?php } ?>
								  </select>
                            </div>
                          </div>
						   ...
						   <div class="field" style="flex: 0 0 calc(33% - 16px);    width: calc(33% - 16px);    margin: 0 8px 24px 8px;">
                            ...
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="total_area" id="total_area" value="<?php echo $displaypropertiesDetail['square_footage']; ?>" placeholder="e. g. 120" style=" color:grey;"  >
                            </div>
                          </div>
                        </div>
                          ...
                          <div class="field" style="flex: 0 0 calc(33% - 16px);    width: calc(33% - 16px);    margin: 0 8px 24px 8px;">
                            <div class="field__label  changedText" style="color:#ffeb3b;">View</div>
                            <div class="field__wrap">
                          ...
                          <div class="field" style="flex: 0 0 calc(33% - 16px);    width: calc(33% - 16px);    margin: 0 8px 24px 8px;">
                            <div class="field__label  changedText" style="color:#ffeb3b;">Condition</div>
                            <div class="field__wrap">
    ...
    color: grey; border-radius: 12px;
      background: transparent; width:100%;">
						  ...
						  <div class="field" style="flex: 0 0 calc(33% - 16px);    width: calc(33% - 16px);    margin: 0 8px 24px 8px;">
                            <div class="field__label  changedText" style="color:#ffeb3b;">Availability</div>
                            <div class="field__wrap">
    ...
    color: grey; border-radius: 12px;
      background: transparent; width:100%;">
								 <option value="Not available" >Select</option>
											<option value="Ready for immediate move-in" >Ready for immediate move-in</option>
					...
					<div class="checkout__row  hidden" style="margin-bottom:24px;">
                          <div class="field" style="margin-bottom:0px;">
                            <div class="field__label  changedText" style="color:#ffeb3b;">Building Class</div>
                            <div class="field__wrap">
    ...
    color: grey; border-radius: 12px;
      background: transparent; width:100%;" >
							   <option value="Not available" >Select</option>
								   <option value="Luxury" class="changedText">Luxury</option>
									<option value="Mid-range" class="changedText">Mid-range</option>
                          ...
                          <div class="field" style="margin-bottom:0px;">
                            <div class="field__label  changedText" style="color:#ffeb3b;">Property Furnishings</div>
                            <div class="field__wrap">
    ...
    color: grey; border-radius: 12px;
      background: transparent; width:100%;" >
							    <option value="Not available" >Select</option>
								  <option value="Full furnishing" class="changedText" <?php if($displaypropertiesDetail['furnished']=="Full furnishing") echo 'selected'; ?>>Full furnishing</option>
								<option value="Kitchen" class="changedText"  <?php if($displaypropertiesDetail['furnished']=="Kitchen") echo 'selected'; ?>>Kitchen</option>
								<option value="Built-in wardrobes" class="changedText"  <?php if($displaypropertiesDetail['furnished']=="Built-in wardrobes") echo 'selected'; ?>>Built-in wardrobes</option>
									<option value="Bathrooms"  <?php if($displaypropertiesDetail['furnished']=="Bathrooms") echo 'selected'; ?>>Bathrooms</option>
									<option value="Partially furnished"  <?php if($displaypropertiesDetail['furnished']=="Partially furnished") echo 'selected'; ?>>Partially furnished</option>
									<option value="Unfurnished"  <?php if($displaypropertiesDetail['furnished']=="Unfurnished") echo 'selected'; ?>>Unfurnished</option>
									<option value="Custom cabinetry"  <?php if($displaypropertiesDetail['furnished']=="Custom cabinetry") echo 'selected'; ?>>Custom cabinetry</option>
									<option value="Conditioner"  <?php if($displaypropertiesDetail['furnished']=="Conditioner") echo 'selected'; ?>>Conditioner</option>
									<option value="Heating"  <?php if($displaypropertiesDetail['furnished']=="Heating") echo 'selected'; ?>>Heating</option>
								  </select>
                            </div>
                          </div>
                        </div>  
						   ...
						   <div class="field hidden">
                            <div class="field__label  changedText" style="color:#ffeb3b;">Annual community fee</div>
                            <div class="field__wrap">
                              <input class="field__input" type="number" name="annual_community_fee" id="annual_community_fee" placeholder="e. g. 120" style=" color:grey;"  >
                            </div>
                          </div>
 ...
 <input type="hidden" id="short_term_rental" name="short_term_rental" value="0"/>
				   <div class="setting__notification hidden">
                      <div class="setting__main">
                        <div class="setting__subtitle" style="color:#ffeb3b;">Short-term Rental Yield</div>
                        <label class="switch">
                          <input class="switch__input" type="checkbox" onclick='updateShortTermRentalInfo(this);'  /><span class="switch__inner"><span class="switch__box"></span></span>
                        </label>
                      </div>
                      <div class="setting__content">Do you want to allow shot term rental</div>
                    </div>
                  
				   <div class="field margin-bottom24 hidden" id="commissionInfo">
 ...
 <input type="hidden" id="long_term_rental" name="long_term_rental" value="0"/>
				   <div class="setting__notification hidden">
                      <div class="setting__main">
                        <div class="setting__subtitle" style="color:#ffeb3b;">Long-term Rental Yield</div>
                        <label class="switch">
                          <input class="switch__input" type="checkbox" onclick='updateLongTermRentalInfo(this);'  /><span class="switch__inner"><span class="switch__box"></span></span>
                        </label>
                      </div>
                      <div class="setting__content">Do you want to allow long term rental</div>
                    </div>
                  
				   <div class="field margin-bottom24 hidden" id="commissionInfo1">
						  ...
						  <div class="field hidden">
                          <div class="field__label" style="color:#ffeb3b;">Detailed Description of the Location</div>
                          <div class="field__wrap">
                            <textarea class="field__textarea" name="detailed_description" id="detailed_description" placeholder="e. g. &quot;Spectacular views of Queenstown&quot;" required=""></textarea>
                          </div>
                        </div>
                          ...
                          <div class="field" style="margin-bottom:0px;">
                            ...
                            <div class="field__wrap">
                               <select class="select"   id="private_entrance" name="private_entrance"   >
								   <option value="Yes" class="changedText">Yes</option>
									<option value="No" class="changedText">No</option>
								  </select>
                            </div>
                          </div>
                          <div class="field" style="margin-bottom:0px;">
					...
					<div class="upload__item">
					  <div class="checkout__line  "  style="margin-bottom:24px;">
					   <div class="checkout__subtitle changedText">Location</div>
					   ...
					   <div class="upload__row">
                        ...
                        <div class="field  ">
                            ...
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="addrs" id="addrs" value="<?php echo $displaypropertiesDetail['address']; ?>" placeholder="e. g. Queenstown, Otago, New Zealand" style=" color:grey;"  readonly>
                            </div>
                          </div>  
						  </div>
						   ...
						   <div class="field">
                            ...
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="pnumber" id="pnumber" placeholder="e. g. 121"  style=" color:grey;"  >
                            </div>
                          </div>
						  </div>
						  </div>
                        ...
                        <div class="field">
                            ...
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="dzip" id="dzip" placeholder="e. g. 120001" value="<?php echo $displaypropertiesDetail['zipcode']; ?>"  style=" color:grey;"  >
                            </div>
                          </div>
						  </div>
						   <div class="upload__col upload__col_w70">
						   <div class="field">
                            ...
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="dcity" id="dcity" value="<?php echo $displaypropertiesDetail['city']; ?>"  placeholder="e. g. Stockholam"  style=" color:grey;"  >
                            </div>
                          </div>
						  </div>
						   </div>
    ...
    color: grey; border-radius: 12px;
      background: transparent; width:100%;">
					    ...
					    <input type='hidden'   id="vitech_region_id" name="vitech_region_id" value="1" />
						<input type='hidden'   id="vitech_city_id" name="vitech_city_id" value="1" />
						<input type='hidden'   id="vitech_area_id" name="vitech_area_id" value="1" />
    ...
    padding: 0 14px;  border-color: #353945;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;"  onchange='selectCity(this.value);' >
								  <option value="0" class="changedText">Select</option>
											<option value="1" class="changedText">Costa Blanca North</option>
											<option value="2" class="changedText">Costa Blanca South</option>
								  </select>
                            </div>
                          </div>
                          <div class="field" style="flex: 0 0 calc(33% - 16px);    width: calc(33% - 16px);    margin: 0 8px;">
    ...
    padding: 0 14px;  border-color: #353945;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;"  onchange='selectArea(this.value);'>
								 <option value="0" class="changedText">Select</option>
								  </select>
                            </div>
                          </div>
						...
						<script>
						  function updateContractStatus(id)
						  {
							  ...
							  if(id==1)
							  {
								...
								$('#homeOwner').removeClass('hidden');
							  }
							  else
								  {
								$('#homeOwner').addClass('hidden');
							  }
						  }
						  
						  </script>
						 <div class="checkout__line  hidden" style="margin-bottom:24px;" >
					   <div class="checkout__subtitle changedText" >Contract info</div>
                       
                      </div>
					 
						    <div class="field hidden" style="margin-bottom:24px;">
                            <div class="field__label  changedText" style="color:#787f91;">Contract type</div>
                            <div class="field__wrap">
                             <select class=" " id="contract_type" name="contract_type" style="height: 48px;
    padding: 0 14px;  border : 2px solid #f67f00;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;" onchange="updateContractStatus(this.value);" >
							  
								   <option value="1" class="changedText">Exclusive</option>
											<option value="2" class="changedText">Not exclusive</option>
                                    </select>
                            </div>
                          </div>
						...
						<div class="upload__item hidden" id="homeOwner">
						 <div class="checkout__line  "  style="margin-bottom:24px;">
					   <div class="checkout__subtitle changedText">Home owner details</div>
                       
                      </div>
						
						 <div class="checkout__row  " style="margin-bottom:24px;">
                        <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">First name</div>
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="first_name" id="first_name"  placeholder="e. g. Kowaken" style=" color:grey;"  >
                            </div>
                          </div>
						  
						   <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">Last name</div>
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="last_name" id="last_name"  placeholder="e. g. Ghirmai" style=" color:grey;"  >
                            </div>
                          </div>
						  </div>
						  
						  <div class="field" style="margin-bottom:24px;">
                            <div class="field__label  changedText" style="color:#787f91;">PERSONAL EMAIL - (NOT WORK)</div>
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="owner_email" id="owner_email"  placeholder="e. g. xyz@gmail.com" style=" color:grey;"  >
                            </div>
                          </div>
                         <div class="upload__row " style="margin-bottom:24px;">
  <div class="upload__col upload__col_w30">
                        <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">Country</div>
                            <div class="field__wrap">
                              <select class=" "   id="country_id" name="country_id"   style="height: 48px;
    padding: 0 14px;  border-color: #353945;
    color: grey; border-radius: 12px;
      background: transparent; width:100%;">
							<?php foreach($countryList as $category => $value) { ?>
											<option value="<?php echo $value['id']; ?>" class="changedText"><?php echo $value['country_name']; ?></option>
											 <?php } ?>
								  </select>
                            </div>
                          </div>
						  ...
						  <div class="upload__col upload__col_w70">
						   <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">MOBILE - (WITHOUT THE FIRST NUMBER 0)</div>
                            <div class="field__wrap">
                              <input class="field__input" type="text" name="uphno" id="uphno"  placeholder="e. g. xxxxx12342" style=" color:grey;"  >
                            </div>
                          </div>
						  </div>
						  </div >
						   ...
						   <div class="checkout__row  " style="margin-bottom:24px;">
                        <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">Start date</div>
                            <div class="field__wrap">
                              <input class="field__input" type="date" name="start_date" id="start_date" min="<?php echo date('Y-m-d'); ?>" value="<?php echo date('Y-m-d'); ?>" placeholder="e. g. Kowaken" style=" color:grey;"  >
                            </div>
                          </div>
						  
						   <div class="field">
                            <div class="field__label  changedText" style="color:#787f91;">End date</div>
                            <div class="field__wrap">
                              <input class="field__input" type="date" name="end_date" id="end_date" min="<?php echo date('Y-m-d'); ?>" value="<?php echo date('Y-m-d'); ?>" placeholder="e. g. Ghirmai" style=" color:grey;" >
                            </div>
                          </div>
						  </div>
