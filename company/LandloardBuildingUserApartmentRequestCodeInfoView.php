...
<head>
	...
	<script>
		...
		function submitform()
		{
			...
			if($("#code_detail").val()==null || $("#code_detail").val()=="")
			{
				$("#error_msg_form").html('Please enter code received in email');
                 submitFlag = 0;
                 return false;
			}
			 
			 if(isNaN($("#code_detail").val())) 
				{
					$("#error_msg_form").html('Code must be a numeric value');
					return false;
				}
				if($("#code_detail").val().length<6 || $("#code_detail").val().length>6) 
				{
					$("#error_msg_form").html('Code must be six digit long');
					return false;
				}
			 var send_data={};
			send_data.code_detail=$("#code_detail").val();	 
				 $.ajax({
							type:"POST",
							url:"../checkRequestInfo/<?php echo $data['cid']; ?>",
							data:send_data,
							dataType:"text",
							...
							success: function(data1){
								...
								if(data1==0)
								{
								$("#error_msg_form").html('No request pending/exist for the given code');
								 submitFlag = 0;
								 return false;	
								}
								 else
			...
			<div class="column_m   talc lgn_hight_22 fsz16 mart40" id="loginBank">
				<div class="wrap maxwi_100 padrl15 xs-padrl25">
					...
					<div class="wi_500p maxwi_100 pos_rel zi5 marrla pad10  xs-pad0">
									...
									<div class="mart10 marb5 xxs-talc talc  xs-padb20 padb35 ffamily_avenir"> <a href="#" class="black_txt fsz25  xs-fsz20 xxs-talc talc edit-text jain_drop_company trn lgn_hight_20">Please enter the code here to fetch the apartment request</a></div>
									
								 
								 
									
							<form action="../verifyRequstDetail/<?php echo $data['cid']; ?>" method="POST" id="save_indexing" name="save_indexing" accept-charset="ISO-8859-1">
								...
								<div class="marb0   padtrl0 white_bg">		 
											...
											<div class="pos_rel">
												...
												<input type="text" name="code_detail" id="code_detail" placeholder="First name" class="white_bg  brdb_new brd_width_2 nobrdt nobrdr nobrdl  tall minhei_65p fsz20 padl10 black_txt ffamily_avenir light_grey_bg floating__input padt25" value="" > 
												
											 <label for="code_detail" class="floating__label tall nobold padl10" data-content="Enter code" >
