...
class ContractorInformationModel extends AppModel
{
	function invitedContractorList($data)
		{
			$dbCon = AppModel::createConnection();
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			 
			$stmt = $dbCon->prepare("select * from company_contractors where company_id=? and is_approved=0");
		...
		function activeContractorList($data)
		{
			$dbCon = AppModel::createConnection();
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			 
			$stmt = $dbCon->prepare("select * from company_contractors where company_id=? and is_approved=1");
			$stmt->bind_param("i", $company_id);
			/* bind parameters for markers */
			$stmt->execute();
			$result = $stmt->get_result();
  ...
  function locationDetail($data)
		{
			$dbCon = AppModel::createConnection();
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			$stmt = $dbCon->prepare("select * from property_location  where property_location.company_id=?");
		...
		{ 
			...
			$dbCon = AppModel::createConnection();
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			$stmt = $dbCon->prepare("select count(id) as num from  company_contractors where `email_address`=? and company_id=? and (is_approved=0 or is_approved=1)");
			$stmt->bind_param("si", $_POST['email_address'],$company_id);
			$stmt->execute();
			$result = $stmt->get_result();
			$row = $result->fetch_assoc();
			$stmt->close();
		...
		function inviteContractor($data)
		{ 
		
			$dbCon = AppModel::createConnection();
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			$stmt = $dbCon->prepare("select id from  company_contractors where `email_address`=? and company_id=? and is_approved=2");
			$stmt->bind_param("si", $_POST['email_address'],$company_id);
			$stmt->execute();
			$result = $stmt->get_result();
			$row = $result->fetch_assoc();
			...
			$stmt = $dbCon->prepare("insert into company_contractors (company_id,first_name,last_name,email_address ,created_on) values (?,?,?,?,now())");
			$stmt->bind_param("isss",$company_id,$first_name,$last_name,$_POST['email_address']);
			$stmt->execute();	
			}
			else
			{
			$stmt = $dbCon->prepare("update company_contractors set is_approved=0 where id=?");
			$stmt->bind_param("i",$row['id']);
			$stmt->execute();		
			}
			$data['email']=$_POST['email_address'];
			$result=$this->sendInvitationEmail($data);
			$stmt->close();
			$dbCon->close();
			return $row['num'];
		}
                ...
                <tr>
                  <td align="left" class="message" valign="top" style="-webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; mso-table-lspace: 0pt; mso-table-rspace: 0pt; border-collapse: collapse; display: table-cell; background-color: rgb(244, 244, 245); padding: 18px;">
                    <div class="text textColorNormal" style="font-family: &quot;GT America Regular&quot;, &quot;Roboto&quot;, &quot;Helvetica&quot;, &quot;Arial&quot;, sans-serif; font-weight: 400; font-size: 16px; line-height: 21px; color: rgb(79, 79, 101);">
                      <span class="text-bold textColorDark" style="font-weight: 600; color: rgb(35, 35, 62);">Contractor invitation</span>
                      <br>You have received a Contractor invitation. To check the detail please restore your Qloud ID app with your email and click on below link to verify the request.</div>
                  </td>
                </tr>
                <tr class="spacer">
                  <td height="12px" colspan="2" style="font-size: 12px; line-height:12px; -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; mso-table-lspace: 0pt; mso-table-rspace: 0pt; border-collapse: collapse; display: table-cell;">&nbsp;</td>
                        ...
                        <td align="center" valign="center" width="100%" style="-webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; mso-table-lspace: 0pt; mso-table-rspace: 0pt; border-collapse: collapse; display: table-cell;">
                          <table border="0" cellspacing="0" cellpadding="0" width="100%" style="-webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; mso-table-lspace: 0pt; mso-table-rspace: 0pt; border-collapse: collapse;">
                            <tbody><tr>
                              ...
                              <a href="https://www.safeqloud.com/user/index.php/ShareMonitor/shareMonitorRequestList" target="_blank" class="Button-primary" style="-webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; font-family: &quot;GT America Medium&quot;, &quot;Roboto&quot;, &quot;Helvetica&quot;, &quot;Arial&quot;, sans-serif; border-radius: 3px; border: 1px solid rgb(32, 32, 192); color: rgb(255, 255, 255); display: block; font-size: 16px; font-weight: 600; padding: 18px; text-decoration: none;"> Manage your invitation </a>                                </td>
                            </tr>
                          </tbody></table>
                        </td>
                      </tr>
