<?php
	...
	class CompanyDepartmentModel extends AppModel
	{
		...
		function countryInfo($data)
	{
		...
		function addedDepartments($data)
		{
			...
			$stmt = $dbCon->prepare("select id,department_name from company_department  where property_id=?  order by created_on desc limit 0,20");
		...
		function moreCompanyDepartments($data)
		{
			$dbCon = AppModel::createConnection();
			$property_id= $this -> encrypt_decrypt('decrypt',$data['lid']);
			$a=$_POST['id']*20;
			$b=20;
			$stmt = $dbCon->prepare("select id,department_name from company_department where property_id=? order by created_on desc limit ?,? ");
											...
											<div class="passport signin_bx dflex fxwrap_w xs-alit_c pos_rel tall">
												...
												<div class="wi_100 xs-wi_100 xs-order_3 xs-padt0 fsz12">
													...
													<div class="fleft wi_50p marr15  "> <div class="wi_50p xs-wi_50p hei_50p xs-hei_50p fxshrink_0 dflex alit_c justc_c uppercase fsz25   brdrad1000 yellow_bg_a lgtgrey_bg  " style="background-repeat: no-repeat;
																	...
																	border-radius: 10%;">'.substr($row['department_name'],0,1).'</div>
																	</div>
													...
													<span class="edit-text bold padt0 jain2 dblock   brdclr_lgtgrey2 fsz14 xs-padt0 black_txt">Department</span>
													  <span class="edit-text   jain2 dblock   brdclr_lgtgrey2 fsz18  black_txt">'.$row['department_name'].'</span>  </div>
													 
													<div class="fright wi_10 padt10 xxs-wi_10 xsip-wi_10 sm-wi_10 marl0 fsz25  padb0 xxs-marr20 hidden-xs">
														<a href="#" class="show_popup_modal padr20" data-target="#edit_wifi" onclick="updatePopup('.$row['id'].','.$dpt.');"><span class="far fa-edit grey_txt"></span> </a>
													</div>
			...
			function addCompanyDepartment($data)
		{
			$dbCon = AppModel::createConnection();
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
			
				$stmt = $dbCon->prepare("INSERT INTO company_department (company_id, created_on , department_name,  property_id ) VALUES (?, now(),  ?, ?)");
				
				
				$stmt->bind_param("isi", $company_id,$_POST['d_name'],$_POST['property_id']);
				if($stmt->execute())
				{
				$stmt->close();
				$dbCon->close();
		...
		function editDepartment($data)
		{
			...
			$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
				...
				$stmt = $dbCon->prepare("update company_department set department_name =? where id=? ");
				$stmt->bind_param("si", $_POST['dname'],$_POST['id']);
