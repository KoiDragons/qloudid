...
class AboutModel extends AppModel
{
  ...
  function completedEmployeeRequests($data)
	 ...
	 function companySummary($data)
    {
		...
		$company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
		//echo $company_id; die;
          $stmt = $dbCon->prepare("select country_name,company_email,company_name,company_categories.name as industry from companies left join country on country.id=companies.country_id left join company_categories on company_categories.id=companies.industry where companies.id=?");
        /* bind parameters for markers */
		$cont=1;
       $stmt->bind_param("i", $company_id);
        $stmt->execute();
