...
class ShareMonitorCompanyModel extends AppModel
{
	...
	function approveRequest($data)
    {
	 ...
	 $id= $this -> encrypt_decrypt('decrypt',$data['id']);
		$stmt = $dbCon->prepare("update company_customer_requests set is_approved=1 where id=?");
	...
	function rejectRequest($data)
    {
        $dbCon = AppModel::createConnection();
	 $id= $this -> encrypt_decrypt('decrypt',$data['id']);
		$stmt = $dbCon->prepare("update company_customer_requests set is_approved=2 where id=?");
