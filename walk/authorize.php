...
if(!checkLogin())
{
	//echo "logout"; die;
	 if(isset($_GET['apply']))
  {
	  header('Location: ../user/index.php/LoginAccount/loginapp?next='.$_GET['client_id'].'&apply=1');
  }
  else  if(isset($_GET['purchase']))
  {
	  header('Location: ../user/index.php/LoginAccount/loginapp?next='.$_GET['client_id'].'&purchase=1');
  }
   else  if(isset($_GET['login']))
  {
	  header('Location: ../user/index.php/LoginAccount/loginapp?next='.$_GET['client_id'].'&login=1');
  }
   else  if(isset($_GET['signin']))
  {
	  header('Location: ../user/index.php/LoginAccount/loginapp?next='.$_GET['client_id'].'&signin=1');
  }
   else  if(isset($_GET['hotel']))
  {
	  header('Location: ../user/index.php/LoginAccount/loginapp?next='.$_GET['client_id'].'&hotel=1');
  }
  else  if(isset($_GET['checkin']))
  {
	  header('Location: ../user/index.php/LoginAccount/loginapp?next='.$_GET['client_id'].'&checkin=1');
  }
}
else
{
...
//echo $query; die;
  ...
  $s=$s.'<div class="wi_360p dtrow brdrad5 mart5 talc">
						<div class="dflex fxdir_col justc_sb alit_c talc brdrad5 lgtgrey2_bg marrl0" id="'.$row1['id'].'">
					
							<a href="#" class="lgtgrey2_bg default_view rbrdr wi_100 mart5 pad10 hei_50p fsz18 llgrey_txt get'.$row1['id'].'" onclick="changeBg('.$row1['id'].');">
...
$query="insert into oauth_check_user (user_id,code,created_on,client_id) values (".$_SESSION['user_id'].",'".$code."',now(),'".$_GET['client_id']."')";
 mysqli_query($dbCon, $query);
		  }
	  ...
	  $query="insert into oauth_check_user (user_id,code,created_on,user_company_id,client_id) values (".$_SESSION['user_id'].",'".$code."',now(),".$_POST['com'].",'".$_GET['client_id']."')";
	   mysqli_query($dbCon, $query);
