...
class SavenameModel extends AppModel
{
    
    function SavenameAccount($data)
    {
        $dbCon  = AppModel::createConnection();
        $stmt   = $dbCon->prepare("Update user_logins set `first_name`=?, `last_name`=?,`sex`=?, `age`=?, `get_started_wizard_status`=?, `country`=?, `state`=?, `city`=?, `district`=?  WHERE `id`=?");
        $status = 1;
            
        /* bind parameters for markers */
        $stmt->bind_param("ssiiiiiiii", $data['fname'], $data['lname'], $data['sex'], $data['age'], $status, $data['cntry'], $data['state'], $data['city'], $data['district'], $_SESSION['user_id']);
