<?php
	...
	class InformRelativesEngModel extends AppModel
	{
		...
		function sendOtp()
		{
			...
			$to=$phone;
			$html='Your one time password(OTP) is:'.$num;
		...
		function verifyOtp()
		{
			...
			if($row['otp']==$_POST['otp'])
			{
				...
				while($row_email = $result1->fetch_assoc())
				{
					...
					$to=$row_email['email'];
					$subject       = 'Information on your next of kin ';
					
					$emailContent ='A person has sent following message about your next of kin, '.$row['name'].'</br>
					
					Message: '.$row['description'].'</br>
					Location: '.$adrs.'</br>
					Phone number: +'.$row['country_code'].' '.$row['sender_phone_number'].'
					';
		...
		function createPopup()
		{
			...
			<span class="fxshrink_0 marl100">Driving license</span>
			</li>
			...
			<label class="marb5  padl10">Select country</label>
			...
			<label class="marb5  padl10">Add your phone number</label>
			<div class="pos_rel padrl10">
			<div class=" wi_28p pos_abs zi2 top0 pos_abs zi2 top0 lef0 padt5 talc lgn_hight_40"><span class="fa fa-star fsz14"></span></div>
			<input type="text" name="pno" id="pno" placeholder="Phone number" max="9999999999" class="white_bg txtind25 wi_100 rbrdr pad10 mart5  white_bg hei_40p black_txt">
