...
class OmOssModel extends AppModel
{
	...
	$stmt = $dbCon->prepare("delete from apps_heading_image where id=?");
	...
	function updateFileName($data)
	{
       ...
       $stmt = $dbCon->prepare("select id from apps_subheading where article_id= ? and company_id=? and user_login_id=?");
		...
		if(empty($row))
		{
			$stmt = $dbCon->prepare("insert into apps_subheading (subheading,article_id,company_id,user_login_id) values (?, ?, ?, ?)");
       
        /* bind parameters for markers */
        $stmt->bind_param("siii", $_POST['title_new'],$_POST['id'],$company_id,$_SESSION['user_id']);
		$stmt->execute();
		}
		else
		{
		$stmt = $dbCon->prepare("update apps_subheading set subheading=? where id=?");
       
        /* bind parameters for markers */
        $stmt->bind_param("si", $_POST['title_new'],$row['id']);
		$stmt->execute();	
		}
   ...
   function appsDetail($data)
    {
        ...
        $stmt = $dbCon->prepare("select apps_heading.id,heading,created_on from apps_heading");
       ...
       //$stmt->bind_param("ii", $company_id,$data['user_id']);
		...
		$i=0;
		 ...
		 {
			 array_push($org,$row);
			 $stmt = $dbCon->prepare("select * from apps_subheading where article_id=? and company_id=? and user_login_id=?");
			...
			$org[$i]['image']=array();
			$stmt = $dbCon->prepare("select * from apps_heading_image where article_id=? and company_id=? and user_login_id=?");
		...
		function getFileName()
	{
		...
		$stmt = $dbCon->prepare("insert into apps_heading_image(image_src,article_id,created_on,img_width,img_height,company_id,user_login_id) 
	values(?, ?, ?, ?, ?, ?, ?)");
