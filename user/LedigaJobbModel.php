...
class LedigaJobbModel extends AppModel
{
	...
	$stmt = $dbCon->prepare("delete from ledigajobb_heading_image where id=?");
	...
	function updateFileName($data)
	{
       ...
       $stmt = $dbCon->prepare("select id from ledigajobb_subheading where article_id= ? and company_id=? and user_login_id=?");
		...
		if(empty($row))
		{
			$stmt = $dbCon->prepare("insert into ledigajobb_subheading (subheading,article_id,company_id,user_login_id) values (?, ?, ?, ?)");
		...
		{
		$stmt = $dbCon->prepare("update ledigajobb_subheading set subheading=? where id=?");
   ...
   function appsDetail($data)
    {
        ...
        $stmt = $dbCon->prepare("select ledigajobb_heading.id,heading,created_on from ledigajobb_heading");
       ...
       //$stmt->bind_param("ii", $company_id,$data['user_id']);
		...
		$i=0;
		 ...
		 {
			 array_push($org,$row);
			 $stmt = $dbCon->prepare("select * from ledigajobb_subheading where article_id=? and company_id=? and user_login_id=?");
			...
			$org[$i]['image']=array();
			$stmt = $dbCon->prepare("select * from ledigajobb_heading_image where article_id=? and company_id=? and user_login_id=?");
		...
		function getFileName()
	{
		...
		$stmt = $dbCon->prepare("insert into ledigajobb_heading_image(image_src,article_id,created_on,img_width,img_height,company_id,user_login_id) 
	values(?, ?, ?, ?, ?, ?, ?)");
