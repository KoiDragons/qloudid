<?php
	...
	class VerifyUserRequestModel extends AppModel
	{
		...
		function verificationId($data)
		{
			$dbCon = AppModel::createConnection();
			$request_id=$this -> encrypt_decrypt('decrypt',$data['request_id']);
			$stmt = $dbCon->prepare("select request_receiver  from user_detail_requests where id=?");
		...
		function userSummary($data)
		{
			...
			$stmt = $dbCon->prepare("select first_name,concat_ws(' ', first_name, last_name) as name,first_name,last_name,email,passport_image,sex,dob_day,dob_month,dob_year,ssn from user_logins left join user_profiles on user_profiles.user_logins_id=user_logins.id where user_logins.id = ?");
			/* bind parameters for markers */
			$cont=1;
			$stmt->bind_param("i", $data['user_id']);
			$stmt->execute();
