...
class UserTenantCompanyModel extends AppModel
{
	...
	function requestDetail($data)
    {
        $dbCon = AppModel::createConnection();
      $company_id= $this -> encrypt_decrypt('decrypt',$data['cid']);
        $stmt = $dbCon->prepare("select user_tenants.id,company_name,last_name,first_name,email,user_tenants.created_on from user_tenants left join user_logins on user_logins.id=user_tenants.user_login_id left join companies on companies.id=user_tenants.company_id where company_id=? and is_approved=0 limit 0,20");
	...
	function moreRequestDetail($data)
    {
	  ...
	  $a=$_POST['id']*20;
	  $b=$a+20;
        $stmt = $dbCon->prepare("select user_tenants.id,company_name,last_name,first_name,email,user_tenants.created_on from user_tenants left join user_logins on user_logins.id=user_tenants.user_login_id left join companies on companies.id=user_tenants.company_id where company_id=? and is_approved=0 limit ?,?");
