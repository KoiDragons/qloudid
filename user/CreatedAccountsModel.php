<?php
	...
	class CreatedAccountsModel extends AppModel
	{
		function moreEvents($data)
		{
			...
			$a=$data['id']*20;
			$b=$a+20;
			$stmt = $dbCon->prepare("select distinct client_company_id from api_user_logs where user_id=? limit ?,?");
			...
			while($row = $result->fetch_assoc())
			{
				...
				$stmt = $dbCon->prepare("select event_name from  `events` where id in(select distinct event from api_user_logs  where client_company_id=?)");
				...
				{
					$org1=$org1.$row2['event_name'].',';
				}
				$org1=substr($org1,0,-1);
				
				
				$org=$org.'<tr>
				
				<td class="pad5 brdb tall">
				<div class="padtb5">'.$row1['company_name'].'</div>
				</td>
				<td class="pad5 brdb tall">
				<div class="padtb5">'.$org1.'</div>
				</td>
		...
		function userEvents($data)
		{
			...
			$stmt = $dbCon->prepare("select distinct client_company_id from api_user_logs where user_id=? limit 0,20");
			/* bind parameters for markers */
			$cont=1;
			...
			while($row = $result->fetch_assoc())
			{
				...
				while($row2 = $result1->fetch_assoc())
				{
					$org[$i]['event']=$org[$i]['event'].$row2['event_name'].',';
				}
				$org[$i]['event']=substr($org[$i]['event'],0,-1);
