<?php
	...
	class SkolaModel extends AppModel
	{
			function studentSummary($data)
		{
			...
			$stmt = $dbCon->prepare("select company_name,user_students.company_id,address from user_students left join companies on companies.id=user_students.company_id left join company_profiles on company_profiles.company_id=companies.id where user_students.user_login_id = ?  and is_approved=1 order by company_name desc limit 0,10");
		...
		function selectIcon($data)
		{
			...
			$stmt = $dbCon->prepare("select app_bg,app_name,app_icon  from apps_detail where id=20");
