<?php
	...
	class LeverantorModel extends AppModel
	{
		function supplierSummary($data)
		{
			$dbCon = AppModel::createConnection();
			
			
			$stmt = $dbCon->prepare("select company_name,user_suppliers.company_id,address from user_suppliers left join companies on companies.id=user_suppliers.company_id left join company_profiles on company_profiles.company_id=companies.id where user_suppliers.user_login_id = ?  and is_approved=1 order by company_name desc limit 0,10");
		...
		function selectIcon($data)
		{
			...
			$stmt = $dbCon->prepare("select app_bg,app_name,app_icon  from apps_detail where id=21");
