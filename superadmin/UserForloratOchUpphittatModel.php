<?php
	...
	class UserForloratOchUpphittatModel extends AppModel
	{
			...
			function forloradDetail()
		{
			...
			$stmt = $dbCon->prepare("select * from user_lost_and_found  where status=1  order by created_on desc limit 0,20");
		...
		function forloradCount()
		{
			...
			$stmt = $dbCon->prepare("select count(id) as num from user_lost_and_found where status=1");
			...
			function moreForloradDetail()
		{
			...
			$a=$_POST['id']*20;
			$b=$a+20;
			$stmt = $dbCon->prepare("select * from user_lost_and_found  where status=1 order by created_on desc limit ?,? ");
		...
		function lostandFoundDetail()
		{
			...
			$stmt = $dbCon->prepare("select * from user_lost_and_found  where status=0  order by created_on desc limit 0,20");
		...
		function lostandFoundCount()
		{
			...
			$stmt = $dbCon->prepare("select count(id) as num from user_lost_and_found where  status=0");
			...
			function moreLostandFoundDetail()
		{
			...
			$a=$_POST['id']*20;
			$b=$a+20;
			$stmt = $dbCon->prepare("select * from user_lost_and_found  where  status=0 order by created_on desc limit ?,? ");
