...
require_once('../AppModel.php');
	...
	class LawCompanyModel extends AppModel
	{
			...
			function approveDeveloper($data)
		{
			...
			$stmt = $dbCon->prepare("update company_lawfirm_account_request set is_approved=1,modified_on=now() where id=?");
			
			/* bind parameters for markers */
			$stmt->bind_param("i", $partner_id);
			$stmt->execute();
			$stmt = $dbCon->prepare("select company_email as email from companies where id = (select company_id from company_lawfirm_account_request where id=?)");
			/* bind parameters for markers */
			$cont=1;
			$stmt->bind_param("i", $partner_id);
			$stmt->execute();
			...
			$to            = $row['email'];
			$subject       = "Law firm request Approved!";
...
">
    ...
    margin-left: auto;  z-index: 5; max-width: 100%; width: auto;">
                              ...
                              <div style="margin-top: 20px; font-weight: normal; margin-bottom: 10px; text-align: center; color:white; font-size: 20px;">Your request for developer account has been approved by superadmin.</div>
                           </div>
                        </div>
                     </div>
  ...
  $stmt->close();
	...
	return 0;
			...
			function rejectDeveloper($data)
		{
			$dbCon = AppModel::createConnection();
			$partner_id= $this -> encrypt_decrypt('decrypt',$data['pid']);
			$stmt = $dbCon->prepare("update company_lawfirm_account_request set is_approved=2,modified_on=now() where id=?");
			...
			$to            = $row['email'];
			$subject       = "Law firm request rejected!";
			$emailContent ='<html><head></head><body><div style="margin-top:0!important;margin-bottom:0!important;margin-right:0!important;margin-left:0!important;padding:0">
...
">
    ...
    margin-left: auto;  z-index: 5; max-width: 100%; width: auto;">
                              ...
                              <div style="margin-top: 20px; font-weight: normal; margin-bottom: 10px; text-align: center; color:white; font-size: 20px;">Your request for developer account has been rejected by superadmin. Please contact support for further information.</div>
                           </div>
                        </div>
                     </div>
  ...
  $stmt->close();
	...
	return 0;
		...
		function developerCount()
		{
			...
			$stmt = $dbCon->prepare("select count(id) as num from company_lawfirm_account_request where is_approved=0");
		...
		function developerDetail()
		{
			...
			$org=array();
				...
				$stmt = $dbCon->prepare("select company_lawfirm_account_request.id,created_on,company_name from company_lawfirm_account_request left join companies on companies.id=company_lawfirm_account_request.company_id where is_approved=0 order by created_on desc limit 0,5");
		...
		function moreLawCompany()
		{
			$dbCon = AppModel::createConnection();
			
			$a=$_POST['id']*5+1;
			$b=5;
			$stmt = $dbCon->prepare("select company_lawfirm_account_request.id,created_on,company_name from company_lawfirm_account_request left join companies on companies.id=company_lawfirm_account_request.company_id where is_approved=0 desc limit ?,? ");
			...
			while($row = $result->fetch_assoc())
			{
				...
				$org=$org.' <div class="column_m container  marb5   fsz14 dark_grey_txt">
												<div class=" white_bg marb0 brdb bg_fffbcc_a" style="">
														...
														<a href="LawCompany/approveDeveloper/'.$org1.'"><span class="fas fa-check-circle green_txt" aria-hidden="true"></span></a>
													</div>
																			<div class="fright wi_10 padl0 xs-wi_20  marl15 fsz40  xs-marl0 xxs-marr20  padb0 ">
														<a href="LawCompany/rejectDeveloper/'.$org1.'"><span class="fas fa-times-circle red_txt" aria-hidden="true"></span></a>
													</div> 
		...
		function developerCountApproved()
		{
			...
			$stmt = $dbCon->prepare("select count(id) as num from company_lawfirm_account_request where is_approved=1");
		...
		function developerDetailApproved()
		{
			...
			$org=array();
				...
				$stmt = $dbCon->prepare("select company_lawfirm_account_request.id,created_on,company_name from company_lawfirm_account_request left join companies on companies.id=company_lawfirm_account_request.company_id where is_approved=1 order by created_on desc limit 0,5");
		...
		function moreLawCompanyApproved()
		{
			$dbCon = AppModel::createConnection();
			
			$a=$_POST['id']*5+1;
			$b=5;
			$stmt = $dbCon->prepare("select company_lawfirm_account_request.id,created_on,company_name from company_lawfirm_account_request left join companies on companies.id=company_lawfirm_account_request.company_id where is_approved=1 desc limit ?,? ");
		 ...
		 function developerCountRejected()
		{
			...
			$stmt = $dbCon->prepare("select count(id) as num from company_lawfirm_account_request where is_approved=2");
		...
		function developerDetailRejected()
		{
			...
			$org=array();
				...
				$stmt = $dbCon->prepare("select company_lawfirm_account_request.id,created_on,company_name from company_lawfirm_account_request left join companies on companies.id=company_lawfirm_account_request.company_id where is_approved=2 order by created_on desc limit 0,5");
		...
		function moreLawCompanyRejected()
		{
			$dbCon = AppModel::createConnection();
			
			$a=$_POST['id']*5+1;
			$b=5;
			$stmt = $dbCon->prepare("select company_lawfirm_account_request.id,created_on,company_name from company_lawfirm_account_request left join companies on companies.id=company_lawfirm_account_request.company_id where is_approved=2 desc limit ?,? ");
