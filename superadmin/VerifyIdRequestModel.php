...
require_once('../AppModel.php');
	...
	class VerifyIdRequestModel extends AppModel
	{
	
			function approveDeveloper($data)
		{
			$dbCon = AppModel::createConnection();
			$partner_id= $this -> encrypt_decrypt('decrypt',$data['pid']);
			$stmt = $dbCon->prepare("update company_idverification_account_request set is_approved=1,modified_on=now() where id=?");
			
			/* bind parameters for markers */
			$stmt->bind_param("i", $partner_id);
			$stmt->execute();
			
			$stmt = $dbCon->prepare("update employees set is_verificator=2 where user_login_id=43 and company_id=(select company_id from company_idverification_account_request where id=?)");
			...
			$stmt = $dbCon->prepare("select email from user_logins where id = (select user_id from company_idverification_account_request where id=?)");
			/* bind parameters for markers */
			$cont=1;
			$stmt->bind_param("i", $partner_id);
			$stmt->execute();
			...
			$to            = $row['email'];
			$subject       = "Verify id request Approved!";
...
">
    ...
    margin-left: auto;  z-index: 5; max-width: 100%; width: auto;">
                              ...
                              <div class="padb0 xxs-padb0 ">
                                 <h1 style="font-size: 50px; font-weight: normal; text-align: center; padding-bottom: 10px; color:white;">Approved</h1>
                              </div>
                              <div style="margin-top: 20px; font-weight: normal; margin-bottom: 10px; text-align: center; color:white; font-size: 20px;">Your request for verify id account has been approved by superadmin.</div>
                           </div>
                        </div>
                     </div>
                              ...
                              <table style="border-spacing:0;font-family:sans-serif;color:#111111;Margin:0 auto;width:100%;table-layout:fixed">
                                ...
                                <tbody><tr>
                                  ...
                                  <td style="padding-top:0px;padding-bottom:0px;padding-left:0px;padding-right:0px;background-color:#ffffff;width:100%;text-align:center">
                                    ...
                                    <p style="Margin-top:0px;line-height:22px;Margin-bottom:0px; fsz14px;">This is sent because your developer account was approved by superadmin,</p>
...
//catch exception
  ...
  $stmt->close();
	...
	return 0;
			...
			function rejectDeveloper($data)
		{
			$dbCon = AppModel::createConnection();
			$partner_id= $this -> encrypt_decrypt('decrypt',$data['pid']);
			$stmt = $dbCon->prepare("update company_idverification_account_request set is_approved=2,modified_on=now() where id=?");
			...
			$to            = $row['email'];
			$subject       = "Verify id request rejected!";
			$emailContent ='<html><head></head><body><div style="margin-top:0!important;margin-bottom:0!important;margin-right:0!important;margin-left:0!important;padding:0">
...
">
    ...
    margin-left: auto;  z-index: 5; max-width: 100%; width: auto;">
                              ...
                              <div style="margin-top: 20px; font-weight: normal; margin-bottom: 10px; text-align: center; color:white; font-size: 20px;">Your request for verify id account has been rejected by superadmin. Please contact support for further information.</div>
                           </div>
                        </div>
                     </div>
                              ...
                              <table style="border-spacing:0;font-family:sans-serif;color:#111111;Margin:0 auto;width:100%;table-layout:fixed">
                                ...
                                <tbody><tr>
                                  ...
                                  <td style="padding-top:0px;padding-bottom:0px;padding-left:0px;padding-right:0px;background-color:#ffffff;width:100%;text-align:center">
                                    ...
                                    <p style="Margin-top:0px;line-height:22px;Margin-bottom:0px; fsz14px;">This is sent because superadmin have taken action on your Developer account request,</p>
  ...
  $stmt->close();
	...
	return 0;
		...
		function developerCount()
		{
			...
			$stmt = $dbCon->prepare("select count(id) as num from company_idverification_account_request where is_approved=0");
		...
		function developerDetail()
		{
			...
			$org=array();
				...
				$stmt = $dbCon->prepare("select company_idverification_account_request.id,created_on,company_name from company_idverification_account_request left join companies on companies.id=company_idverification_account_request.company_id where is_approved=0 order by created_on desc limit 0,5");
		...
		function moreVerifyIdRequest()
		{
			$dbCon = AppModel::createConnection();
			
			$a=$_POST['id']*5+1;
			$b=5;
			$stmt = $dbCon->prepare("select company_idverification_account_request.id,created_on,company_name from company_idverification_account_request left join companies on companies.id=company_idverification_account_request.company_id where is_approved=0 desc limit ?,? ");
				...
				$org=$org.' <div class="column_m container  marb5   fsz14 dark_grey_txt">
												<div class=" white_bg marb0 brdb bg_fffbcc_a" style="">
																<div class="container padrl10 padtb15    brdrad1 fsz18 dark_grey_txt">
																	...
																	<span class="padt0 edit-text jain1 dblock brdclr_lgtgrey2  fsz14 bold  black_txt">Company name</span>
																	
																	 <span class="edit-text  fsz18  jain2 dblock  lgn_hight_18 ">'.substr(html_entity_decode($row['company_name']),0,18).'</span>  
																	</div>
																	
																	
																		<div class="fright wi_10 padl0 xs-wi_20  marl15 fsz40  xs-marl0 xxs-marr20  padb0 ">
														<a href="VerifyIdRequest/approveDeveloper/'.$org1.'"><span class="fas fa-check-circle green_txt" aria-hidden="true"></span></a>
													</div>
																			<div class="fright wi_10 padl0 xs-wi_20  marl15 fsz40  xs-marl0 xxs-marr20  padb0 ">
														<a href="VerifyIdRequest/rejectDeveloper/'.$org1.'"><span class="fas fa-times-circle red_txt" aria-hidden="true"></span></a>
													</div> 
			...
			return $org;
		 	...
		 	function developerCountApproved()
		{
			$dbCon = AppModel::createConnection();
			
			$stmt = $dbCon->prepare("select count(id) as num from company_idverification_account_request where is_approved=1");
		...
		function developerDetailApproved()
		{
			$dbCon = AppModel::createConnection();
			
			$org=array();
			
				$stmt = $dbCon->prepare("select company_idverification_account_request.id,created_on,company_name from company_idverification_account_request left join companies on companies.id=company_idverification_account_request.company_id where is_approved=1 order by created_on desc limit 0,5");
		...
		function moreDeveloperRequestApproved()
		{
			$dbCon = AppModel::createConnection();
			
			$a=$_POST['id']*5+1;
			$b=5;
			$stmt = $dbCon->prepare("select company_idverification_account_request.id,created_on,company_name from company_idverification_account_request left join companies on companies.id=company_idverification_account_request.company_id where is_approved=1 desc limit ?,? ");
		 ...
		 function developerCountRejected()
		{
			$dbCon = AppModel::createConnection();
			
			$stmt = $dbCon->prepare("select count(id) as num from company_idverification_account_request where is_approved=2");
		...
		function developerDetailRejected()
		{
			$dbCon = AppModel::createConnection();
			
			$org=array();
			
				$stmt = $dbCon->prepare("select company_idverification_account_request.id,created_on,company_name from company_idverification_account_request left join companies on companies.id=company_idverification_account_request.company_id where is_approved=2 order by created_on desc limit 0,5");
		...
		function moreDeveloperRequestRejected()
		{
			$dbCon = AppModel::createConnection();
			
			$a=$_POST['id']*5+1;
			$b=5;
			$stmt = $dbCon->prepare("select company_idverification_account_request.id,created_on,company_name from company_idverification_account_request left join companies on companies.id=company_idverification_account_request.company_id where is_approved=2 desc limit ?,? ");
